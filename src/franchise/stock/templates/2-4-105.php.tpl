...
{if $var.error != null}{$var.error}<br>{/if}
</span>
{*--------------- メッセージ類 e n d ---------------*}
...
{*
    {if $smarty.session.shop_div == '1'}
        <tr>
            <td class="Title_Yellow"><b>事業所</b></td>
...
{*
        {if $smarty.session.shop_div == '1'}
            <tr>
                <td class="Title_Yellow">事業所<font color="#ff0000">※</font></td>
                <td class="Value" colspan="3">{$form.form_cshop.html}</td>
            </tr>
        {/if}
*}
...
<table class="List_Table" border="1" width="100%">
    ...
    {/if}
        ...
        {if $var.get_goods_id == NULL && $var.get_ware_id == NULL}
            {* 受払照会 *}
            <td>{$row[$j][1]}<br><a href="2-4-113.php?ware_id={$row[$j][3]}&goods_id={$row[$j][4]}&cshop_id={$var.cshop}&start={$var.hand_start}&end={$var.hand_end}">{$row[$j][2]}</a></td>
        {elseif $var.get_goods_id != NULL && $var.get_ware_id == NULL}
            {* 発注点警告 *}
            <td>{$row[$j][1]}<br><a href="2-4-113.php?ware_id={$row[$j][3]}&goods_id={$row[$j][4]}&cshop_id={$var.cshop}&start={$var.hand_start}&end={$var.hand_end}&trans_flg=1"">{$row[$j][2]}</a></td>
        {elseif $var.get_goods_id != NULL && $var.get_ware_id != NULL}
            {* 在庫照会 *}
            <td>{$row[$j][1]}<br><a href="2-4-113.php?ware_id={$row[$j][3]}&goods_id={$row[$j][4]}&cshop_id={$var.cshop}&start={$var.hand_start}&end={$var.hand_end}&trans_flg=2"">{$row[$j][2]}</a></td>
        {/if}
