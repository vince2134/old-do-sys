 ...
 *
 *
 * ・棚卸は各ショップごとに行う（直営でも他のショップの棚卸は実施できない）
 * ・棚卸日は最終月次更新より前は指定できない
 * ・棚卸日は前回棚卸日より前は指定できない
 *
 * 既知のバグ
...
if($_POST["create_button"] == "調査表作成") {
    ...
    $create_day_y           = $_POST["form_create_day"]["y"];     //棚卸日
    ...
    //直前に他のユーザに調査表を作成されていないかチェック（12-005）
...
//        $sql2 .= ($group_kind == "2") ? " shop_id IN (".Rank_Sql().") " : " shop_id = $shop_id ";
        ...
        /*** 棚卸ヘッダ、棚卸データ登録トランザクション開始 ***/
        Db_Query($db_con, "BEGIN;");

        $pre_ware_id = "";

        /*** 棚卸ヘッダテーブルに倉庫分のデータを登録 ***/
        while($array = pg_fetch_array($result)) {
            ...
            if($pre_ware_id != $array['ware_id']) {
                ...
                // 1.0.1(06-05-09) kajioka-h：棚卸調査表作成時はstaff_idは登録しない
                ...
                $result2 = Db_Query($db_con, $sql2);
                $invent_id = pg_fetch_result($result2, 0, 0);       //棚卸調査ID
            }

            //棚卸データテーブルに倉庫に対する商品分のデータを登録
            ...
            $sql2 .= "    ".$array['book_physical'].", ";   //実棚数はデフォルト帳簿数に戻りました
            //$sql2 .= "    0, ";         //実棚数はデフォルト0に
            $sql2 .= "    '$array[goods_cd]', ";
...
/*
            ...
            //1.0.2(06-05-10) watanabe-k：棚卸調査表作成後は自画面に遷移
...
//「調査取消」ボタンが押された
    ...
    //既に削除、または更新されていないかチェック（12-006、12-007）
    $sql  = "SELECT \n";
...
//初期表示
    ...
    //棚卸更新していない調査表がある場合、棚卸調査表の情報・一覧を表示
    ...
    //棚卸更新していない調査表がない場合、棚卸調査表の作成フォームを表示
    } else {
        $row1 = array($g_today);
...
if($_GET["ware_id"]!=null){
    ...
    $sql .= "t_contents.goods_name,";    //商品名
    $sql .= "t_contents.stock_num,";     //帳簿数
    //$sql .= "t_contents.tstock_num,";    //実棚数
...
/**
 *
 * 棚卸調査表作成後に表示するテーブルのデータ作成
 *
 * @param       resource    $db_con     DBコネクション
 * @param       int         $client_id  取引先ID
 *
 * @return      array       テーブルのデータ
 *
 * @author      kajioka-h <kajioka-h@bhsk.co.jp>
 * @version     1.0.0 (2006/04/27)
 *
...
function Make_Utable($db_con, $client_id) {
    ...
    //1.0.3(06-05-10) watanabe-k：棚卸入力をしていない場合は差異一覧リンクを表示しない。
/*
    $sql2 .= "    CASE";
...
*/
    ...
    $sql2 .= "       WHEN contents2.input_num THEN true ";  //全件数と棚卸実施者が入力されてるレコード数が同じだと入力完了
    $sql2 .= "       ELSE false ";                          //それ以外はまだ
    $sql2 .= "    END AS input_flg, ";

    $sql2 .= "    CASE ";
    $sql2 .= "        WHEN contents2.input_num = contents1.all_num THEN false ";    //全件数と棚卸実施者入力レコード数が同じ（全部入力）は完了
    $sql2 .= "        WHEN contents2.input_num IS NULL THEN false ";                //棚卸実施者入力レコード数がnull（全部未入力）は未
    $sql2 .= "        ELSE true ";                                                  //それ以外（全件数と棚卸実施者入力レコード数が異なる）は一時入力
    $sql2 .= "    END AS temp_flg ";    //一時入力フラグ
