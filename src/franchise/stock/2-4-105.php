...
/****************************/
//変更履歴
//  （2006/05/29）デフォルトで、セッションのショップを表示するように変更
//   (2006/08/03) 商品名を略称から正式名称へ変更
//   (2006/08/08) 全残在庫の抽出SQL変更
//   (2006/08/21) 商品分類での検索処理を追加
//
/****************************/
/*
 * 履歴：
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 * 　2006/10/18　06-007　　　　watanabe-k　URL操作でSQLエラーが表示されるバグの修正
 *  2006/11/22      11-022      ふ          取扱期間検索フォームのエラーチェック修正
 *  2006/11/22      11-023      ふ          取扱期間検索フォームのエラーチェック修正
 *  2006/11/22      11-024      ふ          GET値(goods_id)が不正な場合の処理追加
 *  2006/11/22      11-025      ふ          GET値(ware_id)が不正な場合の処理追加
 *  2006/11/23      11-199~204  ふ          受払データ取得クエリを作り直し
 *  2006/11/23      11-199~204  ふ          受払データ取得クエリを作り直し
 *  2006/12/04      scl_0045    watanabe-k  取扱期間が上と下で異なるバグの修正
 *  2006/12/06      ban_0023    suzuki      日付のゼロ埋め追加
 *  2006/12/14      0055        suzuki      正当性判定修正
 *  2007/02/16                  watanabe-k  事業所を検索項目から削除
 *  2007/02/22                  watanabe-k  不要機能を削除
 *  2007-04-17      B0702-040   kajioka-h   ウィンドウタイトルに画面切替ボタンのHTMLが出力されていたのを修正
 *  2009/10/09                  hashimoto-y 在庫管理フラグをショップ別商品情報テーブルに変更
 *  2009/10/09_1                hashimoto-y 取扱期間の開始日をフォームに変更
 *  2009/10/11                  aoyama-n    会計情報CSV出力機能追加
 *  2009/10/19                  hashimoto-y 取扱期間の終了日をのエラーメッセージ修正漏れ
 *  2009/10/20                  aoyama-n    会計情報CSVのデータとして「移動」を追加
 ...
 *  2011/02/17                  aoyama-n    Rank_Sqlを使用する際の条件を修正（$_SESSION["group_kind"] == "2"）
 *   
 */
...
#"text","sy","テキストフォーム","size=\"4\" maxLength=\"4\" style=\"color : #000000;border : #ffffff 1px solid;background-color: #ffffff; text-align: right;\" readonly" 
#);
$text[] =& $form->createElement(
...
#2009-10-09_1 hashimoto-y
#$text[] =& $form->createElement(
#"text","sm","テキストフォーム","size=\"1\" maxLength=\"2\" style=\"color : #000000;border : #ffffff 1px solid;background-color: #ffffff; text-align: right;\" readonly"
#);
$text[] =& $form->createElement(
...
#2009-10-09_1 hashimoto-y
#$text[] =& $form->createElement(
#"text","sd","テキストフォーム","size=\"1\" maxLength=\"2\" style=\"color : #000000;border : #ffffff 1px solid;background-color: #ffffff; text-align: right;\" readonly"
#);
...
/*
$text[] =& $form->createElement(
"text","sy","テキストフォーム","value=\"2006\" size=\"4\" maxLength=\"4\" style=\"color : #000000;border : #ffffff 1px solid;background-color: #ffffff; text-align: right;\" onkeyup=\"changeText(this.form,'form_hand_day[sy]','form_hand_day[sm]',4)\" readonly"
);
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("text","sm","テキストフォーム","value=\"06\" size=\"1\" maxLength=\"2\" style=\"color : #000000;border : #ffffff 1px solid;background-color:#ffffff; text-align: right;\" onkeyup=\"changeText(this.form,'form_hand_day[sm]','form_hand_day[sd]',2)\" readonly"
);
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("text","sd","テキストフォーム","value=\"30\" size=\"1\" maxLength=\"2\" style=\"color : #000000;border : #ffffff 1px solid;background-color:         #ffffff; text-align: right;\" onkeyup=\"changeText(this.form,'form_hand_day[sd]','form_hand_day[ey]',2)\" readonly"
);
*/
$text[] =& $form->createElement("static","","","　〜　");
$text[] =& $form->createElement(
...
//メニューから遷移の場合
	//表示
//form->addElement("submit","show_button","表　示","onClick=\"javascript:Which_Type('form_output','2-4-106.php','#');\"");
	$form->addElement("submit","show_button","表　示","");
	//クリア
	$form->addElement("button","clear_button","クリア","onClick=\"javascript:location.href='2-4-105.php'\"");
...
//	$form->addElement("submit","show_button","表　示","onClick=\"javascript:Which_Type('form_output','2-4-106.php','#');\"");
	$form->addElement("submit","show_button","表　示","");
	//クリア
	$form->addElement("button","clear_button","クリア","onClick=\"javascript:location.href='2-4-105.php?ware_id=".$get_ware_id."&goods_id=".$get_goods_id."'\"");
}else{
//発注点警告から遷移の場合
	//表示
	$form->addElement("button","show_button","表　示","onClick=\"javascript:Button_Submit('show_button_flg','2-4-105.php','true')\"");	
	//閉じる
...
$form->addElement("hidden", "h_g_product_id");       //商品分類
...
//表示ボタン押下処理
/****************************/
if(count($_POST) > 0 && ($_POST["show_button"] == "表　示" || $_POST["show_button_flg"] == true)){
	//現在のページ数初期化
	$page_count = null;
	$offset = 0;
...
/*
	...
	$sql .= "    t_stock_total.ware_id,";         //在庫集計の倉庫ID
	$sql .= "    t_stock_total.goods_id,";        //在庫集計の商品ID
    $sql .= "    COALESCE(t_stock_total.renew_num,0), \n";       //月次更新前在庫
	$sql .= "    t_stock_total.in_count - COALESCE(t_stock_total.renew_num,0),";        //在庫集計の入庫合計数
	$sql .= "    COALESCE(t_stock_total.out_count,0), ";      //在庫集計の出庫合計数
    $sql .= "    COALESCE(t_stock_total.renew_num,0) \n";
    $sql .= "       +\n";
