 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/11/25　0096　　　　 watanabe-k　GETを書き換えれば、本部でFCの情報をみることができてしまう。
 * 　2006/11/25　0097　　　　 watanabe-k　GETに文字列を渡すとクエリエラーが表示される。
 * 　2006/11/25　0099　　　　 watanabe-k　仕入先名を半角OR全角スペースのみで登録できる
 *   2006-12-08  ban_0086     suzuki      ログに登録するマスタ名変更
 *   2007-02-19  -            morita-d    担当支店を登録できるように修正
 *   2007-02-22  -            watanabe-k  不要機能の削除
 *   2007-05-07  -            watanabe-k　状態ステータスの変更
 *   2007-06-08  -            watanabe-k　取引区分を現金に指定した場合支払日を当月の締日とするように修正
...
*/
    ...
    $select_sql .= "    '',";     //口座番号（※不要となったためデータの抽出はしないが、処理に影響するため空白を抽出）
    $select_sql .= "    '',";     //口座名義（※不要となったためデータの抽出はしないが、処理に影響するため空白を抽出）
...
//    $select_sql .= "    attach_gid";       //FCグループ
    ...
    if($group_kind == '2'){
        $select_sql .= "    AND shop_id IN (".Rank_Sql().")";
    }else{
        $select_sql .= "    AND shop_id = $shop_id";
    }
    $select_sql .= ";";
    //クエリ発行
    ...
    $head_flg                                 = $client_data[44];        //本部フラグ
    $defa_data["form_address3"]               = $client_data[45];        //
    $defa_data["form_b_bank_name"]            = $client_data[46];        //
    $defa_data["form_client_name2"]           = $client_data[47];        //仕入先名2
	$defa_data["form_client_read2"]           = $client_data[48];        //仕入先名2(フリガナ)
...
if($head_flg == 't'){
    //仕入先コード
    $form->addElement(
            "text","form_client_cd","","size=\"7\" maxLength=\"6\" style=\"$g_form_style;border:none;\" readonly"
            );

    //仕入先名1
    $form->addElement(
            "text","form_client_name","","size=\"44\" maxLength=\"25\" style=\"border:none;\" readonly"
            );
    //仕入先名2
    $form->addElement(
            "text","form_client_name2","","size=\"44\" maxLength=\"25\" style=\"border:none;\" readonly"
            );
}else{
    //仕入先コード
    $form->addElement(
...
//資本金
$form->addElement(
        "text","form_capital","","class=\"money\" size=\"11\" maxLength=\"9\" style=\"$g_form_style;text-align: right;\"
        "." $g_form_option"
        );
...
$form->addElement('select', 'trade_ord', 'セレクトボックス', $select_value6,
    "onKeyDown=\"chgKeycode();\" onChange =\"window.focus();trade_buy_close_day()\"");

//締日
$select_value7 = Select_Get($conn,'close');
$form->addElement('select', 'form_close', 'セレクトボックス', $select_value7,$g_form_option_select."onChange =\"trade_buy_close_day()\"");
...
// 全角/半角スペースのみチェック
$form->registerRule("no_sp_name", "function", "No_Sp_Name");
$form->addRule("form_client_name", "仕入先名 にスペースのみの登録はできません。", "no_sp_name");
...
if($_POST["button"]["entry_button"] == "登　録"){
    ...
    $address3       = $_POST["form_address3"];                  //住所２
    ...
    $b_bank_name    = $_POST["form_b_bank_name"];               //振込銀行
    ...
    //支払日が当月で日付が締日より小さい場合
    if($trade_ord == '71' && ($_POST["form_close"] != $_POST["form_pay_d"] || $_POST["form_pay_m"] != '0')){
        $close_err = "取引区分に現金を指定した場合の支払日は当月の締日にして下さい。";
        $err_flg = true;

    }elseif($trade_ord == '21' && $_POST["form_pay_m"] == "0" && ($_POST["form_close"] >= $_POST["form_pay_d"])){
        $close_err = "支払日に締日以前の日付は選択できません。";
        $err_flg = true;
    }
...
if($_POST["button"]["entry_button"] == "登　録" && $form->validate() && $err_flg != true ){
    ...
    /******************************/
        ...
        $update_sql .= "    address3 = '$address3',";           //住所２
        ...
        $update_sql .= "    b_bank_name = '$b_bank_name',";     //金額：丸め区分
...
if($_POST["ok_button_flg"]==true){
    header("Location: ./2-1-216.php");
}

//ボタン
//変更・一覧
$form->addElement("button","change_button","変更・一覧","onClick=\"javascript:Referer('2-1-215.php')\"");
...
if($complete_flg == true){
    ...
    if ($get_client_id == null){
        $sql    = "SELECT MAX(client_id) FROM t_client WHERE shop_id = $shop_id AND client_div = '2';\n";
    ...
    $button[] = $form->createElement(
            ...
            "onClick='javascript:location.href = \"".$_SERVER["PHP_SELF"]."?client_id=$get_id\"'
            ");
    //ＯＫ
    $button[] = $form->addElement(
            "button","ok_button","Ｏ　Ｋ","onClick=\"javascript:Button_Submit_1('ok_button_flg', '#', 'true')\" $disabled"
            );
}else{
    ...
    if ($get_client_id != null){
        $button[] = $form->createElement(
                "button","back_button","戻　る",
                "onClick='javascript:location.href = \"./2-1-215.php\"'
                ");
    }

    //次へボタン
    if($next_id != null){
        $form->addElement("button","next_button","次　へ","onClick=\"location.href='./2-1-216.php?client_id=$next_id'\"");
    }else{
        $form->addElement("button","next_button","次　へ","disabled");
    }
    //前へボタン
    if($back_id != null){
        $form->addElement("button","back_button","前　へ","onClick=\"location.href='./2-1-216.php?client_id=$back_id'\"");
    }else{
...
$contract .= "      var close_day = document.dateForm.form_close.value\n";
$contract .= "      document.dateForm.form_pay_m.value='0';\n";
$contract .= "      document.dateForm.form_pay_d.value=close_day;\n";
$contract .= "  } \n";
$contract .= "}\n";
