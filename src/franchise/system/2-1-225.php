 ...
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 *  2006-12-11      ban_0141    suzuki      CSV出力時にはサニタイジングを行わないように修正
...
if ($_GET["trans_id"] != null && Get_Id_Check_Db($conn, $_GET["trans_id"], "trans_id", "t_trans", "num", $where) != true){
...
$form->addElement(
        ...
        "onClick=\"javascript:Button_Submit_1('input_button_flg', '#', 'true')\""
        );
//登録ボタン
$form->addElement(
...
$form->addElement(
        ...
        "onClick=\"javascript:Button_Submit('csv_button_flg', '#', 'true');\"");
...
$form->addRule("form_trans_name", "運送業者名 にスペースのみの登録はできません。", "no_sp_name");
