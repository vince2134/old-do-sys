 ...
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 *  -----------------------------------------------------------
 * 　2006/12/28　xx-xxx　　　　kajioka-h   グループCD、得意先CDでソートするように変更
 *   2007/01/09  0043          kajioka-h   月次更新テーブルに残す担当者名を契約担当1〜2に修正
 *               0044          kajioka-h   得意先名1〜2、略称、スタッフ名（契約担当）にaddslashes追加
 *   2007/04/11                watanabe-k  一括入金機能追加による影響の修正（一括入金済みの得意先には残高設定できない）
 *   2007/04/19                watanabe-k  一括入金で入金額がnullの場合は売掛残高初期設定を可能とする。 
...
$form->addElement(
    ...
    "onClick=\"javascript:return Dialogue('登録します。','./2-1-305.php', this)\" $disabled"
);
...
*/
    ...
    if($err_flg != true){
        ...
        if($err_mess != null){
            ...
            if($close_day_y."-".$close_day_m."-".$close_day_d <= $close_day_last){
                $form->setElementError("form_close_day","残高以降年月日 は、前回月次更新年月日 より先の日付を入力してくださ>い。");
                $err_flg = true; 
            }       
        }       
    }
