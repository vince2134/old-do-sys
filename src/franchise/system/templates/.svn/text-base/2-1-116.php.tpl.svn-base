...
<table class="Data_Table" border="1" width="650">
	...
	<tr>
		...
		<td class="Value" width="250">{$var.main_goods_cd}　{$var.main_goods_name}</td>
...
<table class="Data_Table" border="1" width="650">
	...
	{foreach key=i from=$loop_num item=items}
		<tr>
			...
			{else}
				<!-- 契約登録から遷移 -->
				<td class="Value">{$form.break_goods_cd[$var.row][$i].html}({$form.form_search[$var.row][$i].html})<br>{$form.break_goods_name[$var.row][$i].html}</td>
			{/if}
			<td class="Value">{$form.break_goods_num[$var.row][$i].html}</td>
			<td class="Value">{$form.break_trade_price[$var.row][$i].html}<br>{$form.break_sale_price[$var.row][$i].html}</td>
			<td class="Value">{$form.break_trade_amount[$var.row][$i].html}<br>{$form.break_sale_amount[$var.row][$i].html}</td>
			</td>
		</tr>
	{/foreach}
