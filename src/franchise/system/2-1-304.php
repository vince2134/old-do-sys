...
/******************************/
//  変更履歴
//      DB構成変更に伴う修正（20060906）
//
//  (2006-09-15) (kaji)
//  ・DB構成変更に伴う修正
//  ・月次更新後は初期設定できないように変更
//
//　（2006-10-10）（watanabe-k）
//　・pay_m　→　pay_out_m
//　・pay_d　→　pay_out_d
//
/******************************/
 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/11/23　0084　　　　  watanabe-k　システム開始日チェック追加
 * 　2007/01/05　　　　　 　　 watanabe-k　本部側で登録している初期残高を抽出しないように修正
 *   2007/01/29  xx-xxx        kajioka-h   月次買掛残高テーブルに取引先区分を残すように変更
 *   2007/01/30  xx-xxx        watanabe-k  2重登録チェック時にも本部側で登録している初期残高を抽出しないように修正
 *   2007/07/12  xx-xxx        watanabe-k  残高移行日を表示するように修正 
...
$form->addElement(
    ...
    "onClick=\"javascript:return Dialogue('登録します。','./2-1-304.php', this)\" $disabled"
);
...
*/
            ...
            //取引先区分を抽出
            $sql  = "SELECT client_div FROM t_client WHERE client_id = $client_id[$i]";
            $result = Db_Query($conn, $sql);
            $client_div = pg_fetch_result($result, 0,0);
            ...
            //FCの場合
            if($client_div == '3'){
                $sql .= " LEFT JOIN t_staff AS t_staff1 ON t_client.sv_staff_id = t_staff1.staff_id ";
            //仕入先の場合
            }else{
                $sql .= " LEFT JOIN t_staff AS t_staff1 ON t_client.c_staff_id1 = t_staff1.staff_id ";
            }
            $sql .= "
                WHERE 
                    t_client.client_id = $client_id[$i] 
...
//  $sql .= "    ON t_client.client_id = t_payout.client_id\n";
    ...
    $sql .= "    AND\n";
//  $sql .= "    t_payout.client_id IS NULL\n";
    $sql .= "    t_payout_h.client_id IS NULL\n";
    $sql .= "    AND\n";
    $sql .= "    t_first_ap_balance IS NULL\n";
    $sql .= $where_sql;
/*
    //直営の場合のみ
    if($group_kind == "2"){
