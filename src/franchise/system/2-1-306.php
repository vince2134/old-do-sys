 ...
 * 　2007/04/11　      　　　　watanabe-k　一括入金機能追加による影響範囲
 *   2007/04/19                watanabe-k  一括入金の入金額がnullの場合は初期残高は登録可能とする。
 *   2007/06/14                watanabe-k  請求先１を登録後、請求先２が登録できないバグの修正
 *   2007/06/22                watanabe-k  前受金のチェック処理を追加
 */
...
*/
    ...
    if($form->validate() && $err_flg != true && count($add_client_id)> 0){
        ...
        for($i = 0; $i < count($add_client_id); $i++){
            ...
            $sql .= "   AND \n";
            $sql .= "   t_bill_d.claim_div = '$add_claim_div[$i]' \n";
            $sql .= ";";
...
if($_POST["renew_flg"] == 1){
    ...
    $sql .= "           LEFT JOIN \n";
    $sql .= "       t_advance \n";
    $sql .= "       ON t_client.client_id = t_advance.client_id \n";
    $sql .= "       AND t_advance.claim_div = t_claim.claim_div \n";
    ...
    $sql .= "       AND\n";
    $sql .= "       t_advance.client_id IS NULL \n";
    $sql .= "       AND \n";
