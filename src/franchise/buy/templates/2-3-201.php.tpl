...
{*+++++++++++++++ メッセージ類 begin +++++++++++++++*}
    ...
    {/if}
    {if $var.buy_ord_num_err != null}
        <li>{$var.buy_ord_num_err}<br>
    {/if}
...
<table>
    <tr>
        <td>{$form.form_client_button.html}　{$form.form_fc_button.html}</td>
    </tr>
</table>
        </td>
    </tr>
...
<table width="100%">
    <tr>
        {if $var.client_search_flg == true && $var.stock_search_flg == true}
...
<table width="100%">
    <tr>
		...
		{* 登録確認画面判定 *} 
		{if $var.client_search_flg == true && $var.stock_search_flg == true && $var.comp_flg != true}
			{* 以外 *} 
        	<td align="right">{$form.form_buy_button.html}</td>
		{elseif $var.comp_flg == true}
			{* 登録確認画面 *} 
