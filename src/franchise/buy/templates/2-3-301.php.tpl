...
{*if $form.form_order_all_check.error != null   --- 2007.06.14#コメント--- *}
...
<col>
    ...
    <tr>
        ...
        <td class="Td_Search_1" colspan="3">{$form.form_client.html} {$form.form_client_name.html}</td>
    </tr>
    <tr>
...
{/if}
    ...
    {/if}
        ...
        <td align="center">{$row[$j][14]}</td>
	{if $row[$j][13] == '2'}
	        <td align="left">{$row[$j][0]}<br>{$row[$j][12]}<br>
	{else}
        	<td align="left">{$row[$j][0]}-{$row[$j][1]}<br>{$row[$j][12]}<br>
        {/if}
        ...
        <td align="center"><a href='2-3-308.php?pay_id={$row[$j][11]}&c_id={$row[$j][15]}' target="_self">明細</a></td>
