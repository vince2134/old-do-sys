...
/****************************/
//　変更履歴
//      （20060920）月次更新日・仕入締日の日付チェックを追加＜Watanabe-K＞
//       2006/10/23 07-001 kaku-m 一括設定で日付が空欄の場合00がセットされないように修正
//       2006/10/23 07-006 kaku-m 手形支払は銀行必須のチェックするようにを修正
//       2006/10/23 07-007 kaku-m 登録時に銀行のデータを保持する変数を初期化するように修正
//       2006/10/24 07-013 kaku-m 登録データチェックの取得配列の変更・チェック項目の追加・修正
//       2006/10/25 07-005 kaku-m hidden変数・処理を追加し、メッセージ表示を修正
//       2006/10/26 kaku-m 登録確認画面時、登録行以外を空白に、銀行入力がないとき、コンボボックスを非表示に修正
//       2006/10/26 kaku-m 予定一覧画面からの変数受け渡しをSESSIONに変更
//       2006/10/26 kaku-m GETの値チェックを追加
//       2006/12/01 kaku-m 支払先、支払日（月まで）の変更時、手数料をクリアするように修正
//       2007/01/22 kaku-m 区分ラジオボタンをクリックしたときにイベント発生するように修正
//       2007/01/22 kaku-m ダイアログを開くときに、FCか仕入先かの値を受け渡すように変更
//       2007/01/22 kaku-m 仕入先番号の「-」が正しく表示されるように修正
//       2007/01/29 kaku-m 日時更新済みのデータの表示が正しくなかったものを修正
//       2007/03/27 kaku-m 日付のエラーチェック後にmktimeの実行場所を変更
//       2007/03/28 kaku-m 確認画面で、金額をナンバーフォーマットするように修正
/****************************/

/*
 * 履歴：
 * 　日付　　　　B票No.　　　  担当者　　　内容　
 * 　2006/11/29　scl_0012　　　watanabe-k　フリーズ後の金額をナンバーフォーマットしていないバグの修正
 * 　2006/12/10　scl_0064　　　watanabe-k　直営以外のFCで画面を開いた場合にエラーが表示されるバグの修正
 * 　2006/12/10　scl_0061　　　watanabe-k　フラグの初期化処理を追加
 * 　2006/12/16　scl_0084　　　watanabe-k　変更時に支払予定額が表示されないバグの修正
 * 　2007/07/03　        　　　watanabe-k　支払予定IDが複数ある場合にクエリエラーが表示されるバグの修正
 *  2007-07-12                  fukuda      「支払締」を「仕入締」に変更
 *  2007-07-13                  fukuda      支払金額の取得を、仕入先選択時に行う
...
if ($_GET["pay_id"] != null){
    ...
    // GETした支払IDの妥当性チェック
    $safe_flg = Get_Id_Check_Db($db_con, $_GET["pay_id"], "pay_id", "t_payout_h", "num", "shop_id IN ".Rank_Sql2()."");
    if ($safe_flg === false){
...
/****************************/
// ヘッダ部リンクボタン
$ary_h_btn_list = array("照会・変更" => "./2-3-303.php", "入　力" => $_SERVER["PHP_SELF"]);
...
if ($_POST["confirm_button"] == "支払確認画面へ" || $_POST["payout_flg"] == "true"){
    ...
    for ($key = 0, $row = 0; $key < $max_row; $key++){
        ...
        if (!in_array($key, $ary_del_rows)){
            ...
            // ■仕入先
            // 必須チェック１
            if ($ary_client_id[$key] == null && $post_client_cd1[$key] != null){
    ...
    if ($get_flg == "true"){
        ...
        $num  = pg_num_rows($res);
        if ($num == 0){
            header("Location: ./2-3-305.php?err=1");
            exit;
        }
    }
    ...
    if ($get_flg == "true"){
        ...
        $num  = pg_num_rows($res);
        if ($num == 0){
            header("Location: ./2-3-305.php?err=2");
            exit;
        }
    }
    ...
    if ($_POST["payout_flg"] == "true" && $err_flg != true){
        ...
        foreach ($ary_input_id_row as $key){
            ...
            if ($get_flg != "true"){
                ...
                if ($_SESSION["group_kind"] == "2"){
                    ...
                    if ($res === false){
                        ...
                        $err_format = "t_payout_no_serial_pkey";    // ユニーク制約の制約名
                ...
                $sql .= "       AND \n";
                $sql .= "           t_schedule_payment.shop_id = ".Rank_Sql2()." \n";
                $sql .= "       AND \n";
                $sql .= "           t_schedule_payment.first_set_flg = 'f' ";
                $sql .= "   ), \n";
                $sql .= "   $shop_id, \n";
            ...
            for ($i = 0; $i < $d_insert_cnt; $i++){
                ...
                // 新規支払時
                    ...
                    $sql .= "   (SELECT pay_id FROM t_payout_h WHERE pay_no = '$pay_no_max' AND shop_id IN ".Rank_Sql2()."), \n";
                }
                // 支払金額用
                if ($i == 0){
        ...
        if ($err_flg != true && $duplicate_flg != true){
            // 完了画面へ遷移
            header("Location: ./2-3-305.php?flg=".$_POST["get_flg"]."&pay_id=".$pay_id);
            exit;
        }
...
for ($i = 0; $i < $max_row; $i++){
    ...
    if (!in_array($i, $ary_del_rows)){
        ...
        if ($disp_pattern == "3"){
            ...
            // 仕入先コード２
            // 仕入先検索ダイアログからの戻り値（null）セット用にhiddenで作成しておく
            $form->addElement("hidden", "form_client_cd2_$i", "", "
                size=\"4\" maxLength=\"4\" class=\"ime_disabled\"
                onChange=\"Change_f2layer(this.form, 'layer', '#$row_num', $i);\"
                $g_form_option
            ");
