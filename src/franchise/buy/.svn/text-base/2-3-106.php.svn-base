...
/***************************
変更履歴
    チェックボックスを作成するfor分のmaxを$result→$num変更
    2006/11/29  出荷予定数が受注数を満たない発注だと表示がおかしくなるのを修正
***************************/
/*
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/10/11　06-015　　　　watanabe-k　発注残一覧で発注完了のチェックボックスが表示されないバグの修正
 *   2006/10/18  06-012        watanabe-k　年月日の入力チェックで文字列が入力された場合が考慮されていないバグの修正
 */
 ...
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 *  2006/11/13      03-087      ふ          仕入完了済発注データの強制完了を行わないよう修正
 *  2006/11/13      03-114      ふ          チェックボックスと発注データIDの同期をとるよう修正
 *  2006/11/13      03-122      ふ          削除または変更されている発注データに強制完了できるバグを修正
 *  2006/11/13      06-123      suzuki      商品名を省略しないように修正
 *  2006/11/29      scl_104-2.[FC]  suzuki  分納条件指定を修正
 *  2006/12/06      ban_0039    suzuki      日付にゼロ埋め追加
 *  2006/12/06                  watanabe-k  強制完了の条件を変更
 *  2007/02/06                  watanabe-k  発信日の表示を修正
 *  2007-03-30                  fukuda      検索条件復元処理追加
 *  2007-04-25                  watanabe-k  倉庫での検索を可能に
...
// 本部出荷予定日（開始〜終了）
Addelement_Date_Range($form, "form_arrival_day", "本部出荷予定日", "-");
...
// 発注残一覧ボタン
$form->addElement("button", "ord_button", "発注残一覧", $g_button_color."onClick=\"location.href='$_SERVER[PHP_SELF]'\"");

// 入力ボタン
$form->addElement("button", "new_button", "入　力", "onClick=\"javascript:Referer('2-3-102.php')\"");
...
if($_POST["ord_comp_button_flg"]== "true"){
    ...
    for($c=0; $c<count($_POST["hdn_ord_d_id"]); $c++){
        ...
        if($row_num[$c]==1){
            ...
            if (pg_num_rows($res) > 0){
                ...
                //・必須入力チェック
                if($reason[$c] == null){
                    $form->setElementError("err_non_reason", "発注完了理由 は必須です。");
                    $ord_err_flg = true;
                }
    ...
    // 発注完了ボタン押下時にエラーがある場合
        ...
        // 発注完了チェックボックスと完了理由のPOSTをフォームに補完
    ...
    if (count($ary_data) > 0){
        foreach ($ary_data as $key => $value){
            ...
            if (array_key_exists("aord_arr_day_join", $value)){
                ...
                // No.がある場合
                if ($no != null){
                    // 発注状況「取消」または、発注状況「null」かつ処理状況「未処理」の場合
...
*/
                ...
                // No.がある場合
                if ($no != null){
                    $buy_link       = "<a href=\"./2-3-201.php?ord_id=".$value["ord_id"]."\">入力</a>";
                }else{
                    $buy_link       = null;
                }
                // ■発注完了理由
...
*/
                ...
                $html_l .= "            理由：".$form->_elements[$form->_elementIndex["form_reason[$row]"]]->toHtml()."<br>";
                $html_l .= "        </td>\n";
                $html_l .= "    </tr>\n";
...
$html_s .= "        <td class=\"Td_Search_3\">本部出荷予定日</td>\n";
