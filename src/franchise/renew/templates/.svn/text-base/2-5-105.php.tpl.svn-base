...
{/if}
{if $form.form_branch.error != null}
    <li>{$form.form_branch.error}<br>
{/if}
</ul>
</span>
...
{* 日次更新対象外メッセージ *}
{if $print.err_not_renew != null}
<span style="color: #ff0000; font-weight: bold; line-height: 130%;">
<ul style="margin-left: 16px;">
    <li>{$print.err_not_renew}<br>
</ul>
</span>
{/if}
{*--------------- メッセージ類 e n d ---------------*}
...
<col>
    ...
    <tr>
        ...
        <td class="Value">{$print.start_day} 〜 {$print.end_day}</td>
    </tr>   
    <tr>
        <td class="Title_Green">支店</td> 
        <td class="Value">{$form.form_branch.html}</td>
    ...
    <tr>
        ...
        <td class="Value">{$print.staff_name}</td>
    </tr>
    {* 代行明細時 *}
...
{* ページ移動ボタン *}
{if $smarty.get.id == null}
    <div>{$form.form_list_button.html}</div><br>
