 ...
 *  2007-05-07      -           fukuda      入力された得意先の請求先を取得
 *  2009-10-15      rev.1.3     kajioka-h   得意先の請求先を表示する
...
// 得意先リンク
$form->addElement("link", "form_client_link", "", "#", "得意先",
...
//rev.1.3 請求先情報
for($i=1; $i<=2; $i++){
	$form->addElement("hidden", "hdn_claim_close_day[$i]");
	$form->addElement("hidden", "hdn_claim_pay_m[$i]");
	$form->addElement("hidden", "hdn_claim_pay_d[$i]");
}
...
if ($_POST["client_search_flg"] == "true"){
    ...
    if ($num > 0){
        ...
        $client_cname   = pg_fetch_result($res, 0, 1);      // 得意先名（略称）
    }else{  
        $client_id      = "";   
        $client_cname   = "";   
    }
...
if ($_POST["form_show_button"] != null){
    ...
    if ($num > 0){
        ...
        //$close_day      = pg_fetch_result($res, 0, 4);	//rev.1.3 請求先の情報を表示するためコメントアウト
        //$pay_m          = pg_fetch_result($res, 0, 5);	//rev.1.3 請求先の情報を表示するためコメントアウト
        //$pay_d          = pg_fetch_result($res, 0, 6);	//rev.1.3 請求先の情報を表示するためコメントアウト
...
//print_array($sql);
        ...
        while($claim_list = pg_fetch_array($result)){
            $claim_name[$claim_list[5]]			= htmlspecialchars($claim_list[0]."　".$claim_list[1]);	//請求先CD　請求先名
			$claim_close_day[$claim_list[5]]	= $claim_list[2];	//請求先締日
			$claim_pay_m[$claim_list[5]]		= $claim_list[3];	//請求先集金日（月）
			$claim_pay_d[$claim_list[5]]		= $claim_list[4];	//請求先集金日（日）
        }
    ...
    if ($err_flg != true){
		...
		//rev.1.3 請求先情報をhiddenにセット
		foreach($claim_name as $claim_div => $value){
			$hdn_set["hdn_claim_name"][$claim_div]		= $claim_name[$claim_div];
    	    $hdn_set["hdn_claim_close_day"][$claim_div]	= $claim_close_day[$claim_div];
        	$hdn_set["hdn_claim_pay_m"][$claim_div]     = $claim_pay_m[$claim_div];
	        $hdn_set["hdn_claim_pay_d"][$claim_div]     = $claim_pay_d[$claim_div];
		}
...
// ページ遷移時の処理
/****************************/
// 表示ボタン未押下かつ得意先検索フラグnullかつページ数がPOSTされている場合
if ($_POST["form_show_button"] == null && $_POST["client_search_flg"] == null && $_POST["f_page1"] != null){
	...
	//rev.1.3 hiddenの請求先情報を変数に代入
	foreach($_POST["hdn_claim_name"] as $claim_div => $value){
		$claim_name[$claim_div]			= $_POST["hdn_claim_name"][$claim_div];
		$claim_close_day[$claim_div]	= $_POST["hdn_claim_close_day"][$claim_div];
		$claim_pay_m[$claim_div]		= $_POST["hdn_claim_pay_m"][$claim_div];
		$claim_pay_d[$claim_div]		= $_POST["hdn_claim_pay_d"][$claim_div];
	}
...
*/
	...
	foreach($claim_name as $claim_div => $value){
	    // 締日
    	$claim_close_day[$claim_div] = ($claim_close_day[$claim_div] == "29") ? "月末" : $claim_close_day[$claim_div]."日";

	    // 集金日
    	$claim_pay_m[$claim_div] = ($claim_pay_m[$claim_div] == "0")  ? "当月"  : $claim_pay_m[$claim_div];
	    $claim_pay_m[$claim_div] = ($claim_pay_m[$claim_div] == "1")  ? "翌月"  : $claim_pay_m[$claim_div];
    	$claim_pay_m[$claim_div] = ($claim_pay_m[$claim_div] != "当月" && $claim_pay_m[$claim_div] != "翌月") ? $claim_pay_m[$claim_div]."ヶ月後" : $claim_pay_m[$claim_div];
	    $claim_pay_d[$claim_div] = ($claim_pay_d[$claim_div] == "29") ? "月末"  : $claim_pay_d[$claim_div]."日";
	}
...
if ($post_show_flg == true && $err_flg != true &&  $_POST["form_output"] == 2){
    ...
    $csv_file_name = "得意先先元帳_" .$csv_client_cname .$end_y .$end_m .$end_d .".csv";
...
if (($post_show_flg == true && $err_flg != true) || $post_page_flg == true){
	...
	//rev.1.3 請求先表示追加
	if($claim_name[1] != null){
    $html_2 .= "    <tr>\n";
    $html_2 .= "        <td class=\"Title_Pink\">請求先1</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_name[1]."</td>\n";
    $html_2 .= "        <td class=\"Title_Pink\">締日</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_close_day[1]."</td>\n";
    $html_2 .= "        <td class=\"Title_Pink\">集金日</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_pay_m[1]."の".$claim_pay_d[1]."</td>\n";
    $html_2 .= "    </tr>\n";
	}
	if($claim_name[2] != null){
    $html_2 .= "    <tr>\n";
    $html_2 .= "        <td class=\"Title_Pink\">請求先2</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_name[2]."</td>\n";
    $html_2 .= "        <td class=\"Title_Pink\">締日</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_close_day[2]."</td>\n";
    $html_2 .= "        <td class=\"Title_Pink\">集金日</td>\n";
    $html_2 .= "        <td class=\"Value\">".$claim_pay_m[2]."の".$claim_pay_d[2]."</td>\n";
    $html_2 .= "    </tr>\n";
	}
    //$html_2 .= "    </tr>\n";
    $html_2 .= "</table>\n";
    ...
    $html_2 .= "    </tr>\n";
    $html_2 .= "    <tr>\n";
    $html_2 .= "        <td class=\"Title_Pink\">前受金残高</td>\n";
    ...
    $html_3 .= "        <td class=\"Title_Pink\">担当者".(($_SESSION["group_kind"] == "2") ? "<br>（代行先）" : null)."</td>\n";
