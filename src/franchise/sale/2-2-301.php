...
/****************************/
// ・変更履歴
//      請求データのない得意先の請求先について請求書を作成しないバグを修正（watanabe-k）
//      (20060929)バグ管理台帳'0021'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0022'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0023'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0024'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0025'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0026'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0027'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0028'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0029'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0030'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0031'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0032'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0033'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0034'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0035'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0036'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0037'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0038'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0039'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0040'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0041'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0042'の修正（watanabe-k）
//      (20060930)バグ管理台帳'0043'の修正（watanabe-k）
/****************************/
/*
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/10/20　04-001　　　　watanabe-ｋ 7月分の請求更新後、8月分の請求書を作成すると「前回請求額」が正しくないバグの修正。            
 * 　2006/10/20　04-002　　　　watanabe-ｋ 7月分の請求更新後、8月分の請求書を作成すると「今月御支払額」が正しくないバグの修正。            
 * 　2006/10/23　04-011　　　　watanabe-ｋ 日次更新していないデータがあった場合も請求データを作成してしまうバグの修正            
 * 　2006/11/14　04-127　　　　watanabe-ｋ 請求先１，２を指定し請求データを作成した場合に請求先２の請求書が作成されていないバグの修正            
 * 　2006/11/14　04-128　　　　watanabe-ｋ 請求先１，２を指定し請求データを作成した場合に当月支払額が合計されているバグの修正
 * 　2006/12/04　ssl-0037　　　watanabe-ｋ 請求書がないにもかかわらず、エラーが表示されるバグの修正
 * 　2006/12/04　指摘事項　　　watanabe-ｋ 請求日をもとに日次更新チェックを行っていたが、売上日を見るように修正
 * 　2007/01/04　指摘事項　　　watanabe-ｋ 随時の場合のチェック変更
 * 　2007/02/06　指摘事項　　　watanabe-ｋ ラジオボタンの文言変更
 * 　2007/02/06　指摘事項　　　watanabe-ｋ 年月分を削除
 * 　2007/02/14　        　　　watanabe-ｋ 一括消費税の処理を追加
 *   2007/03/23                watanabe-k  担当支店での絞りこみ機能を追加
 *   2007/04/04                watanabe-k  親子関係を残すように修正
 *   2007/04/10                watanabe-k  エラーメッセージを表示するように修正
 *   2007/04/11                watanabe-k  一括入金の金額を抽出するように修正
 *   2007/04/16                watanabe-k  請求書作成時に確定フラグをtにする
 *   2007/04/17                watanabe-k  税込割賦除外額を登録
 *   2007/04/17                watanabe-k  御買上残高の計算を繰越残高＋税込買上額-割賦売上額+今回割賦請求額
 *   2007/05/08                watanabe-k  対象買上額の抽出条件を変更
 *   2007/06/04                watanabe-k  請求月の請求先に対して請求書を作成できるように修正
 *   2007/06/13                watanabe-k  請求データIDが重複するバグの修正 
 *   2007/06/19                watanabe-k　随時作成の得意に日付を入力できないように修正 
 *   2007/06/21                watanabe-k　前受金の処理を追加 
 *   2007/07/03                watanabe-k　取引がある場合にも金額が０であれば請求書を作成しないように修正
 *   2007/07/04                watanabe-k　日次更新チェック時に請求先区分をみていないバグの修正
 *   2009/07/30                aoyama-n    随時個別作成時、請求締日で作成済みの場合にエラーメッセージが表示されないバグ修正　
...
$form->addElement('select', 'form_branch_id', '本支店', $branch_value, $g_form_option_select);
...
//$add_create_button_flg = ($_POST["form_create_button"] == "作　成")? true : false;
...
if($slipout_type == '1'){
    ...
    //本支店
    $form->addRule("form_branch_id","本支店を選択して下さい。","required");
    ...
    $branch_id          = $_POST["form_branch_id"];        //本支店
...
//日はマスタの締日を使用するので、日付のチェックのためにとりあえず1日をセット
    ...
    if($form->validate() && $claim_day_flg == false && $claim_err_flg == false){
        ...
        //aoyama-n 2009-07-30
        //随時個別作成時、請求締日で作成済みの場合にエラーメッセージが表示されない
        //SQLを実行していないため処理追加
        $result = Db_Query($db_con, $sql);
...
if($system_start_flg == true){
    $form->setElementError("form_claim_day2","請求締日に'2005年1月1日より前の日付は入力できません。");
    $err_flg = true;
}
...
//            $sql = "EXECUTE duplicate_date_check(".$client_id.",'".$client_close_day."', '".$client_claim_div."')"; 
            ...
            if($num >0){
//                $form->setElementError("form_claim_day1","指定した締日に対する請求データは作成済みです。");
//                $duplicate_err = true;
                continue;
            }
...
//print_array($sql, "SQL");
            ...
            $c_client_name[$j]              = $claim_client_data["client_name"];                    //[5-4]取引先名
            $c_client_name2[$j]             = $claim_client_data["client_name2"];                   //[5-5]取引先名２
            ...
            $bill_close_day_last[$j]        = ($claim_client_data["bill_close_day_last"] != null)? $claim_client_data["bill_close_day_last"] : START_DAY ;  //[6-1]前回請求日
            ...
            if($claim_data["tax_div"] == '1'){
                ...
                if($claim_data["c_tax_div"] == '1'){
                    //[9-1] + [9-2] + [8-4]
                    $sale_amount[$j]    = $taxation_amount[$j] + $notax_amount[$j] + $installment_net_amount[$j];   //[13-4]今回買上額
                    //([9-1] * [0-1] / 100) + [8-5]
                    $tax_amount[$j]     = bcadd(bcmul($taxation_amount[$j], $tax_rate,2), $installment_tax_amount[$j], 2);  //[13-5]今回消費税額（税まるめ前）
                ...
                #}else{
                    //[9-1] + [9-2] + [8-4] + [8-5]
                    #$intax_amount[$j]   = $taxation_amount[$j] + $notax_amount[$j] + $installment_net_amount[$j] + $installment_tax_amount[$j]; //[13-6]税込御買上額

                    //[9-1] - ([9-1] / ( 1 + [0-1])) + [8-5]
                    #$t_amount_data[$j]  = bcmul($taxation_amount[$j], $tax_rate,2);                 //[9-1] / ( 1 + [0-1])
                    #$tax_amount[$j]     = bcadd(bcsub($taxation_amount[$j], $t_amount_data[$j]), $installment_tax_amount[$j], 2);//[13-5]今回消費税額（税まるめ前）
...
//               $lump_tax_amount[$j]    = ($receivable_tax_amount[$j] + $installment_tax_amount[$j] - $tax_amount[$j]) * -1;
            ...
            //・請求先の課税単位が「伝票単位」の場合
                ...
                //[8-1]+[8-4]
                $sale_amount[$j]    = $receivable_net_amount[$j] + $installment_net_amount[$j];         //[13-4]今回買上額

                //[8-2]+[8-5]
                $tax_amount[$j]     = $receivable_tax_amount[$j] + $installment_tax_amount[$j];         //[13-5]今回消費税額
...
//            $bill_amount_this[$j]           = $rest_amount[$j] + $intax_amount[$j];                         //[13-8]今回請求額
            //この金額には今回の割賦支払額は入っていないため、割賦支払額抽出後に再度計算する。
            ...
            $advance_balance_this[$j]   = $advance_ary["advance_total_this"];   //今回前受金残高額
            $advance_balance_last[$j]   = $advance_ary["advance_total_last"];   //前回前受金残高額
            $advance_amount[$j]         = $advance_ary["advance_total"];        //当月前受金額
            $advance_offset_amount[$j]  = $advance_ary["advance_payin_total"];  //当月前受金相殺額
...
/*
print "<hr>";
print "得意先：".$c_client_cd1[$j]."-".$c_client_cd2[$j]."<br>";
print "税込買上額：".$all_intax_amount[$j]."<br>";
print "割賦売上額：".$all_installment_sales_amount[$j]."<br>";
print "分割請求額：".$split_bill_amount[$j]."<br>";
print "前回支払額：".$payment_last[$j]."<br>";
print "今回入金額：".$pay_amount[$j]."<br>";
//print "今回支払額：".$pay_amount_this[$j]."<br>";
*/	                                                    
...
/*
        ...
        print "合計";
        print "<hr>";
        print "前回請求額：".$sum_bill_amount_last;
        print "<br>";
        print "今加入金額：".$sum_pay_amount;
        print "<br>";
        print "調整額：".$sum_tune_amount;
        print "<br>";
        print "繰越残高額：".$sum_rest_amount;
        print "<br>";
        print "当月御買上額：".$sum_sale_amount;
        print "<br>";
        print "当月御買上消費税額：".$sum_tax_amount;
        print "<br>";
        print "当月税込御買上額：".$sum_intax_amount;
        print "<br>";
        print "割賦売上額：".$sum_installment_sales_amount;
        print "<br>";
        print "分割請求額：".$sum_split_bill_amount;
        print "<br>";
        print "分割請求残高額：".$sum_installment_receivable_balance;
        print "<br>";
        print "今回請求額：".$sum_pay_amount_this;
        print "<br>";
        print "今回支払額：".$sum_sales_slip_num;
*/ 
