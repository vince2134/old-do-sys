 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/11/29  scl-0005　　　watanabe-k　親子関係有で、明細請求書を指定しているのに、合計請求書のみが表示されるバグの修正
 * 　2007/03/30          　　　watanabe-k　合計請求書に小計、合計を表示
 * 　2007/04/26          　　　morita-d  　「親子なし」かつ「個別明細」で2ページ目以降は合計金額を表示しないように修正
 * 　2007/06/16          　　　watanabe-k　請求書を選択していない場合はエラー表示
 * 
 */
...
foreach($claim_issue AS $key => $bill_id){
	...
	if($bill_id != "f"){
		...
		//分割請求額に含まれる伝票の番号を抽出
		//$div_sale_data = Get_Div_Sale($db_con,$bill_id);
		//if(in_array($div_sale_data)){
			//$bill_data[$bill_id][head][div_sale] .= implode(",", $div_sale_data);
		//}
