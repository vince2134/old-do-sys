...
{*                <td align="right">{if $claim_data.payment_this>0}{$claim_data.payment_this|number_format}{else}0{/if}</td>*}
    ...
    <tr>
        {if $claim_data.advance_balance != 0}
        <td align="left">
        <table class="List_Table" border="1" width="">
            <tr class="Result1">
                <td class="Title_Pink" width="" ><b>{$smarty.const.ADBANCE_BALANCE}</b></td>
                <td width="100" align="right">{$claim_data.advance_balance|number_format}</td>
            </tr>
        </table>
        </td>
        {/if}
...
{*割賦がある場合のみ*}
    ...
    <tr class="Result1">
        ...
        {if $bill_d_data[$i][$j].position == 2}
        <td align="right">{$split_bill_data[$i][$j].formal_name}</td>
        {else}
        <td align="left">{$split_bill_data[$i][$j].formal_name}</td>
        {/if}
        ...
        <td align="right">{$split_bill_data[$i][$j].pay_amount}</td>
        <td align="center"></td>
    </tr>
    {/if}
    {/foreach}
...
{*
    <tr>
        <td align="right">
        {$form.form_add_button.html}{$form.form_slipout_button.html}　　{$form.modoru.html}
        </td>
    </tr>
*}
</table>
