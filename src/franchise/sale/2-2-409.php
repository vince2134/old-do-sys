 ...
 *  2009/06/15      改修No.4    aizawa-m	取引区分：「相殺」=> 「銀行」 必須にしないチェック追加
 *	2009/06/16		改修No.4	aizawa-m	取引区分：「相殺」=> 「銀行」 は選択できないチェック追加
 *  2009/07/27                  aoyama-n    請求先を変更すると請求締日以前の日付で入金できてしまう不具合修正
 *  2011/02/05                  watanabe-k  請求書作成以後に、入金できてしまう不具合の修正
 *  2016/02/02                  amano    IE11 でアンカータグへのジャンプが利かなくなったことの対応
 */
...
//        $sql .= "   t_client.state = '1' \n";
        $sql .= "AND \n";
        $sql .= ($_SESSION["group_kind"] == "2") ? " t_client.shop_id IN (".Rank_Sql().") " : " t_client.shop_id = $shop_id ";
        $sql .= ";";
...
/*
            ...
            $sql .= "       AND \n";
            $sql .= ($_SESSION["group_kind"] == "2") ? " t_bill.shop_id IN (".Rank_Sql().") " : " t_bill.shop_id = $shop_id ";
            $sql .= "   ) \n";
            $sql .= "   AS t_bill_max \n";
            ...
            if ($num > 0){
                ...
                // 該当請求先に対して1回目の請求の場合は、支払予定額集計期間（開始）データがnullになっているレコードがあったため
                // null→システム開始日に置き換える
...
if ($_POST["form_ok_btn"] != null){
    ...
    foreach ($ary_insert_forms as $key => $i){
        ...
        $sql .= "   'f', \n";
        $sql .= "   NULL, \n";
        $sql .= ($group_kind == "2") ? " (SELECT cclient_shop FROM t_client_info WHERE client_id = $post_claim_id[$i]) \n" : " $shop_id \n";
        $sql .= ") \n";
        $sql .= ";";
        $res  = Db_Query($db_con, $sql);
        // エラー時はロールバック
        ...
        $sql .= "   pay_no = '$payin_no' ";
        $sql .= "AND ";
        $sql .= ($group_kind == "2") ? " shop_id = (SELECT cclient_shop FROM t_client_info WHERE client_id = $post_claim_id[$i]) " : " shop_id = $shop_id ";
        $sql .= ";";
        $res  = Db_Query($db_con, $sql);
    ...
    if (!in_array($i, $ary_del_row_history)){
        ...
        if ($verify_flg != true){
            ...
            $form->addElement("link", "form_claim_search[$i]", "", "#", "検索",
                "tabindex=\"-1\" 
                 onClick=\"javascript:return Open_SubWin_3('../dialog/2-0-250.php',
            ...
            // 入金日
            Addelement_Date($form, "form_payin_date[$i]", "入金日", "-");
            ...
            // 手形期日
            Addelement_Date($form, "form_limit_date[$i]", "手形期日", "-");
...
if($verify_flg != true){
    ...
    // 得意先単位ボタン
    $form->addElement("button", "form_trans_client_btn", "得意先単位", "onClick=\"location.href('2-2-402.php')\"");

    // 銀行単位ボタン
    $form->addElement("button", "form_trans_bank_btn", "銀行単位", $g_button_color." onClick=\"location.href('2-2-409.php')\"");
...
if ($verify_flg == true){
    ...
    // 入金OKボタン
    $form->addElement("submit", "hdn_form_ok_btn", "入金ＯＫ", "onClick=\"Double_Post_Prevent2(this);\"  $disabled");
...
for ($i=0, $j=0; $i<$max_row; $i++){
    ...
    if (!in_array($i, $ary_del_row_history)){
        ...
        // IE11ではテーブルの中のアンカーにはジャンプできない。　2016-02-02 amano
        //$html .= "<a name=\"".$j."\"></a>\n";
        $html .= "  <td align=\"right\" id=\"".++$j."\">$j</td>\n";                                                     // 行番号
        $html .= "  <td>\n";
