...
if($_POST[submit] == "登　録"){
    ...
    //ファイルを開く
    $handle = fopen("./20061215/goods.txt", "r");
    ...
    while($csv_data = fgets($handle, "1000") ){
        ...
        $insert_data["goods_cd"]        = str_pad($insert_data[3], '8', '0', STR_PAD_LEFT);
        $insert_data["goods_name"]      = addslashes(trim($insert_data[4]));
        $insert_data["goods_cname"]     = addslashes(trim($insert_data[5]));
