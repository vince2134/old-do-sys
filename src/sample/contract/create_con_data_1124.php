...
while($con_list = pg_fetch_array($con_result)){
	...
	$cresult = Db_Query($db_con, $sql); 
	$data_list = Get_Data($cresult);
	//契約区分判定
	if($data_list[0][0] != '2'){
		//通常・オフライン伝票作成
		Aorder_Query($db_con,93,$contract_id,$start_day,$end_day);
	}else{	
		//オンライン伝票作成
		Aorder_Query($db_con,93,$contract_id,$start_day,$end_day,$data_list[0][1]);
	}	
}
