...
/****************************/
/*
// 売上、仕入、入金、支払のいずれかのデータがあるオペレータを抽出
$sql  = "SELECT \n";
$sql .= "   t_attach.staff_id, t_staff.staff_name \n";
$sql .= "FROM \n";
$sql .= "   ( \n";
$sql .= "       (SELECT e_staff_id FROM t_sale_h WHERE shop_id = $shop_id) ";
$sql .= "       UNION \n";
$sql .= "       (SELECT e_staff_id FROM t_payin_h WHERE shop_id = $shop_id) ";
$sql .= "       UNION \n";
$sql .= "       (SELECT e_staff_id FROM t_buy_h WHERE shop_id = $shop_id) ";
$sql .= "       UNION \n";
$sql .= "       (SELECT e_staff_id FROM t_payout_h WHERE shop_id = $shop_id) ";
$sql .= "   ) \n";
$sql .= "   AS t_e_staff \n";
$sql .= "   INNER JOIN t_attach ON t_e_staff.e_staff_id = t_attach.staff_id \n";
...
$num_all_e_staff_list  = pg_num_rows($res);
if ($num_all_e_staff_list > 0){
    $i = 0; 
    while ($ary_res = @pg_fetch_array($res, $i)){
        $ary_e_staff_list[$i][0]    = $ary_res[0];
        $ary_e_staff_list[$i][1]    = $ary_res[1];
        $e_staff_info[$ary_res[0]]  = $ary_res[1]; //スタッフIDと名前の対応配列を作成
        $i++;
    }
}else{
    $ary_e_staff_list = array();
}
*/
$sql  = "SELECT \n";
$sql .= "   t_attach.staff_id, t_staff.charge_cd, t_staff.staff_name \n";
...
foreach ($ary_e_staff_list as $key => $value){
    $def_fdata["form_e_staff[$key]"] = "1";
}
$def_fdata["form_count_date"]["y"]  = date("Y");
$def_fdata["form_count_date"]["m"]  = date("m");
$def_fdata["form_count_date"]["d"]  = date("d");
...
$text[] =& $form->createElement("text", "y", "",
        "size=\"4\" maxLength=\"4\" style=\"$g_form_style\"
         onkeyup=\"changeText(this.form,'form_count_date[y]', 'form_count_date[m]',4)\"
         onFocus=\"onForm_today(this,this.form,'form_count_date[y]','form_count_date[m]','form_count_date[d]')\"
         onBlur=\"blurForm(this)\""
);
$text[] =& $form->createElement("static", "", "", "-");
$text[] =& $form->createElement("text", "m", "",
        "size=\"1\" maxLength=\"2\" style=\"$g_form_style\"
         onkeyup=\"changeText(this.form,'form_count_date[m]', 'form_count_date[d]',2)\"
         onFocus=\"onForm_today(this,this.form,'form_count_date[y]','form_count_date[m]','form_count_date[d]')\"
         onBlur=\"blurForm(this)\""
);
$text[] =& $form->createElement("static", "", "", "-");
...
foreach ($ary_e_staff_list as $key => $value){
    // 5人目で折り返す
    $return_staff = (bcmod($i, 5) == 0) ? "<br>" : null;
    $form->addElement("checkbox", "form_e_staff[$key]", null, $value[1].$return_staff);
    $i++;
}
*/
...
// ■日付としての妥当性チェック
/*
$form->registerRule("check_date_qf", "function", "Check_Date_Qf");
$form->addRule("form_count_date", $err_msg, "check_date_qf");
*/
...
if (!in_array("form_count_date", $ary_addrule_err_forms) && $close_date_plus != null && $_POST["form_show_button"] != null &&
    $close_date_plus > $_POST["form_count_date"]["y"]."-".$_POST["form_count_date"]["m"]."-".$_POST["form_count_date"]["d"]){
    $form->setElementError("form_count_date", "集計日に 前回の月次締日より前の日付が指定されています");
}
*/
if (!in_array("form_count_date", $ary_addrule_err_forms) && $close_date_plus != null && $_POST["form_show_button"] != null &&
...
/****************************/
/*
// オペレータをチェック。DBに一人でも登録されていればtrue
$all_query_flg = ($num_all_e_staff_list != 0) ? true : false;

// オペレータリストが空でない場合
if ($all_query_flg == true){

    // 全オペレータIDをカンマ区切りで取得(SQLの条件文　INで使用する)
    foreach ($ary_e_staff_list as $key => $value){
        $all_staff_csv .= $value[0];
        $all_staff_csv .= ($key+1 < count($ary_e_staff_list)) ? ", " : "";
        $ary_all_e_staff_id[] = $value[0];
    }
    ...
    if ($_POST["form_show_button"] == null){
        ...
        //全オペレータを選択したものとする
        $select_staff_csv = $all_staff_csv;
        $select_query_flg = true;
        $ary_select_e_staff_id = $ary_all_e_staff_id;

    // 初期表示でない　かつ　オペレータが選択された場合
    } elseif (count($_POST["form_e_staff"]) != "0") {

        // 選択オペレータを配列で取得
        foreach ($_POST["form_e_staff"] as $key => $value){
            $ary_select_e_staff_id[] = $ary_e_staff_list[$key][0];
        }
        // 選択オペレータIDをカンマ区切りで取得(SQLの条件文　INで使用する)
        foreach ($ary_select_e_staff_id as $key => $value){
            $select_staff_csv .= $value;
            $select_staff_csv .= ($key+1 < count($ary_select_e_staff_id)) ? ", " : "";
        }
        $select_query_flg = true;   
...
// 集計日作成
if ($verify_flg == true){
    $count_date = $_POST["form_count_date"]["y"]."-".$_POST["form_count_date"]["m"]."-".$_POST["form_count_date"]["d"];
}else{
...
if ($select_query_flg == true){
    ...
    $sql .= "           INNER JOIN t_staff ON t_attach.staff_id = t_staff.staff_id \n";
    $sql .= "       WHERE \n";
    $sql .= "           t_attach.staff_id IN (";
    foreach ($ary_select_e_staff_id as $key => $value){
        $sql .= $value;
        $sql .= ($key+1 < count($ary_select_e_staff_id)) ? ", " : ") \n";
    }
    $sql .= "       AND \n";
    ...
    $sql .= "           COALESCE(t_sale_h.net_amount, 0) - COALESCE(t_sale_h.cost_amount, 0) AS margin \n";
    $sql .= "       FROM \n";
    $sql .= "           t_sale_h \n";
...
if ($all_query_flg == true){
    ...
    $sql  = "
        SELECT
            NULL,
            '全社',
            t_sale_data.c_part_cd,
            t_sale_data.c_staff_name,
            t_sale_data.sale_day,
            SUM(t_sale_data.net_genkin),
            SUM(t_sale_data.net_kake),
            SUM(t_sale_data.net_genkin + t_sale_data.net_kake) AS net_amount,
            SUM(t_sale_data.tax_genkin),
            SUM(t_sale_data.tax_kake),
            SUM(t_sale_data.net_genkin + t_sale_data.tax_genkin) AS genkin_amount,
            SUM(t_sale_data.net_kake + t_sale_data.tax_kake) AS kake_amount,
            SUM(t_sale_data.net_genkin + t_sale_data.net_kake + t_sale_data.tax_genkin + t_sale_data.tax_kake) AS all_amount,
            SUM(t_sale_data.net_genkin + t_sale_data.net_kake-t_sale_data.cost_amount)
        FROM 
        ( 
            SELECT 
                t_attach.staff_id AS e_staff_id, 
        ...
        (
            ...
            SELECT 
            t_sale_h.e_staff_id,
            t_sale_h.e_staff_name,
            t_sale_h.c_staff_id,
            t_sale_h.c_staff_name,
            t_staff.charge_cd AS c_staff_cd,
            t_part.part_cd AS c_part_cd,
            t_sale_h.sale_day,
            ...
            SUM( 
                CASE
                    ...
                    THEN
                        COALESCE(t_sale_h.net_amount, 0) * 1
                    WHEN
                        t_sale_h.trade_id = 63 OR
                        t_sale_h.trade_id = 64
                    THEN
                        COALESCE(t_sale_h.net_amount, 0) * -1
                    ELSE 0
                END
            ...
            SUM( 
                CASE
                    WHEN
                        t_sale_h.trade_id = 11 OR
                        t_sale_h.trade_id = 15
                    THEN
                        COALESCE(t_sale_h.net_amount, 0) * 1
                    WHEN
                        t_sale_h.trade_id = 13 OR
                        t_sale_h.trade_id = 14
                    THEN
                        COALESCE(t_sale_h.net_amount, 0) * -1
                    ELSE 0
                END
            ...
            SUM( 
                CASE
                    WHEN
                        t_sale_h.trade_id = 61
                    THEN
                        COALESCE(t_sale_h.tax_amount, 0) * 1
                    WHEN
                        t_sale_h.trade_id = 63 OR
                        t_sale_h.trade_id = 64
                    THEN
            ...
            SUM( 
                CASE
                    WHEN
                        t_sale_h.trade_id = 11 OR
                        t_sale_h.trade_id = 15 OR
                        t_sale_h.trade_id = 61
                    THEN
                        COALESCE(t_sale_h.cost_amount, 0) * 1
                    WHEN
                        t_sale_h.trade_id = 13 OR
                        t_sale_h.trade_id = 14 OR
                        t_sale_h.trade_id = 63 OR
                        t_sale_h.trade_id = 64
                    THEN
                        COALESCE(t_sale_h.cost_amount, 0) * -1
            ...
            t_sale_h
            LEFT JOIN t_attach
            ON t_sale_h.c_staff_id = t_attach.staff_id
            AND t_sale_h.shop_id = t_attach.shop_id
            LEFT JOIN t_staff
            ON t_sale_h.c_staff_id = t_staff.staff_id
            LEFT JOIN t_part
            ON t_attach.part_id = t_part.part_id
            
            WHERE t_sale_h.shop_id = $shop_id
--            AND t_sale_h.renew_flg = 'f'
            AND t_sale_h.".(($count_type == "1") ? "enter_day" : "sale_day")." >= '$count_date_s 00:00:00'
            AND t_sale_h.".(($count_type == "1") ? "enter_day" : "sale_day")." <= '$count_date_e 23:59:59'
            GROUP BY 
            t_sale_h.e_staff_id,
            t_sale_h.e_staff_name,
            t_sale_h.c_staff_id,
            ...
            ORDER BY t_sale_h.c_staff_id,t_sale_h.sale_day
        ) AS t_sale_data
        ON t_attach_e_staff.e_staff_id = t_sale_data.e_staff_id
        --WHERE t_sale_data.c_part_cd IS NOT NULL
        --AND t_sale_data.c_staff_name IS NOT NULL
        GROUP BY 
            t_sale_data.c_part_cd, 
            t_sale_data.c_staff_cd,  
            t_sale_data.c_staff_name,  
            t_sale_data.sale_day
...
--                    AND t_buy_h.renew_flg = 'f'
                ...
                ON t_attach_e_staff.e_staff_id = t_buy_data.e_staff_id
--                WHERE t_buy_data.c_part_cd IS NOT NULL
...
if ($select_query_flg == true){
    ...
    $sql  = "(SELECT e_staff_id AS stafd_id FROM t_sale_h   $whe) UNION \n";
    ...
    $sql  = "(SELECT e_staff_id AS stafd_id FROM t_sale_h   $whe_sale) UNION \n";
    ...
    foreach ($ary_select_e_staff_id as $e_staff_id){
        ...
        $html .= "<fieldset>\n";
//        $html .= "<legend><span style=\"font: bold 15px; color: #555555;\">【".$e_staff_info[$e_staff_id]."】</span></legend>\n";
