 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 *  2016/01/22                amano  Button_Submit, Dialogue, Dialogue_1, Dialogue_2 関数でボタン名が送られない IE11 バグ対応  
 **************************/
...
//登録ボタン
$button[] = $form->createElement("submit","touroku","登　録","onClick=\"javascript: return Dialogue('登録します。','#', this)\" $disabled");
//削除ボタン
$button[] = $form->createElement("button","delete","削　除","onClick=\"javascript: return Dialogue_2('削除します。','#','true','delete_button_flg', this)\" $disabled");
//表示ボタン
$button[] = $form->createElement("button","display","表　示","onClick=\"javascript:Button_Submit('show_button_flg','#','true', this)\"");
...
for($r=1;$r<=$row;$r++){
	if(!in_array("$r", $d_history)){
		...
		$html_row .= "</td>\n<td align=\"center\">";
		$html_row .= "<a href=\"#\" style=\"color:blue\" onClick=\"javascript: return Dialogue_1('削除します。',".$r.",'delete_row_flg', this)\">削除</a>";
		$html_row .= "</td>\n</tr>\n";
		$row_count++;
	}
}
