...
{if $form.form_handling_day.error != null}<li>{$form.form_handling_day.error}<br>{/if}
</span>
{*--------------- メッセージ類 e n d ---------------*}
...
【取扱期間：
{if $form.form_handling_day.error == null && ($var.hand_start != NULL || $var.hand_end != NULL)}{$var.hand_start} 〜 {$var.hand_end}
{else}指定無し
{/if}
】
</span>
