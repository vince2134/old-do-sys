 ...
 *    2006/07/11 (watanabe kouji)(渡部浩二がめんどくさがって書かずソース管理ができないため、親切なかじおかが代記)
 *      ・マスタの商品を表示ボタン
 *    2006/07/12 (kaji)
 *      ・調整理由をセレクトボックスに変更
 *      ・ボタン名を変更
 ...
 *      ・メッセージを追加
 *      ・完了ボタンへリンクを追加
 *    2006/07/14(watanabe-k)
 *      ・倉庫が選択されていない場合
 *      ・一時的に無効商品も表示
 *    2006/07/18(watanabe-k)
 *      ・倉庫が選択されていない場合は、商品入力フォームを表示しないように変更
 *    2006/07/18(watanabe-k)
 *      ・在庫数を表示する時は指定された日付を考慮する。
 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2007/01/24　仕様変更　　　watanabe-k　ボタン色変更
 * 　2007/03/28　仕様変更　　　watanabe-k　調整理由にメンブレン製造を追加
 *   2007/04/03  B0702-032     kajioka-h   状態が有効の場合に無効商品が使えるバグ修正
 *   2009/10/12                hashimoto-y 在庫管理フラグをショップ別商品情報テーブルに変更
 *   2016/01/22                amano  Button_Submit 関数でボタン名が送られない IE11 バグ対応  
 *   2016/01/22                amano  Html_Header の互換処理を追加
 */
...
$ware_name = stripslashes($_POST["form_ware_name"]);
...
/*
        ...
        $goods_defo_data["form_adjust_num"][$i]         = "";
...
if($_POST["show_button_flg"] == "true"){
    ...
    //エラーフラグ
    $err_flg = ($ware_id == null || $err_flg == true)? true : false;

    //エラーフラグがFALSEの場合のみ処理開始
    if($err_flg == false){
    ...
    $result = Db_Query($conn, $sql);
    $ware_name = htmlspecialchars(pg_fetch_result($result, 0,0));
...
*/
    ...
    $goods_input_data["form_type"]     = $_POST["hidden_type"];
    //input_row_numをクリア
...
//        }
    ...
    $change_data["form_type"]     = $_POST["hdn_type"];
    $adjust_day = explode("-", $_POST["hdn_adjust_date"]);
...
$form->addElement("button","form_all_select_button","倉庫内の全商品を表示","
        onClick=\"javascript:Button_Submit('allgoods_button_flg','#', 'true', this)\"");
$form->addElement("button", "form_show_button","表　示","
        onClick=\"javascript:Button_Submit('show_button_flg','#', 'true', this)\"");
//$form->addElement("button","form_adjust_button","調　整","
//        onClick=\"javascript:Dialogue_2('調整します。','#', 'true', 'adjust_button_flg')\" $disabled");
//$form->addElement("button","form_adjust_button","実　施",
//        "onClick=\"javascript:Dialogue_2('在庫を調整します。','#', 'true', 'adjust_button_flg');\" $disabled");
$form->addElement("submit", "form_adjust_button", "実　施", "onClick=\"javascript:return Dialogue('在庫を調整します。','#')\" $disabled");

$form->addElement("button","change_button","一　覧","
		onClick=\"javascript:Referer('1-4-112.php')\"");
...
$form->addElement("button","mst_goods_button","在庫商品を表示","
        onClick=\"javasclipt:Button_Submit('mst_button_flg', '#', 'true', this)\"");
$form->addElement("button","add_row_button","行追加","onClick=\"javascript:Button_Submit_1('add_row_flg', '#', 'true', this)\"");
...
$form->addElement("hidden", "hdn_type");            //値を保持するためのhidden
$form->addElement("hidden", "hdn_adjust_date");     //値を保持するためのhidden
...
//        $select_id = ($ware_id != null)?"$ware_id" : null;
        /****************************/
        //表示用HTML作成
        /****************************/
if($ware_id != null){
        ...
        $html .=        "（<a href=\"#\" onClick=\"return Open_SubWin('../dialog/1-0-210.php',Array('form_goods_cd[".$i."]','input_row_num'),500,450,1,'$ware_id', '$i');\">検索</a>）";
        $html .=        "<br>";
...
//print_array($_POST["adjust_button_flg"]);
...
//        header("Location: $_SERVER[PHP_SELF]");
        header("Location: 1-4-108-2.php?done_flg=true");
    }
