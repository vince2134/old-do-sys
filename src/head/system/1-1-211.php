 ...
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 *  2006-12-11      ban_0132    suzuki      CSV出力時にはサニタイジングを行わないように修正
...
if($_POST["form_entry_button"] == "登　録"){
    ...
    if($num > 0 && ($update_flg != true || ($update_flg == true && $def_g_goods_cd != $g_goods_cd))){
        $form->setElementError("form_g_goods_cd","既に使用されている Ｍ区分コード です。");
    }

    //コード体系
    if($g_goods_cd >= 5000 && $g_goods_cd < 9000){
        $form->setElementError("form_g_goods_cd","Ｍ区分コードに5000〜8999は利用できません。");
    }
