 ...
 *   2016/01/20                amano  Dialogue, Dialogue_1 関数でボタン名が送られない IE11 バグ対応    
 */
...
if($freeze_flg == true){
	...
	// 入力権限のあるスタッフのみ出力
	$form->addElement("submit","form_entry_button","登　録","onClick=\"javascript:return Dialogue('構成品は一度登録すると変更できません。','#', this)\" $disabled");
    //次へボタン
    if($next_id != null){
...
for($i = 0; $i < $max_row; $i++){    //削除行判定
    if(!in_array("$i", $del_history)){
       ...
       //仕入単価 
        ...
        if($freeze_flg != true && $update_flg != true){
            $html .=    "<td align=\"center\">";
            $html .=       "<a href=\"#\" onClick=\"javascript:Dialogue_1('削除します。',  '$del_data', 'delete_row', this)\">削除</a>";
            $html .=    "</td>\n";
        }
        $html .= "</tr>\n";
