...
//ゲットがない場合は、フォーマットの未表示するため、その判定フラグ
$format_flg = ($GET["ord_id"] == null)? true :false;
...
//    }
	...
	$result = Db_Query($conn,$sql);
	$ord_h_data = pg_fetch_array($result, 0);

	$shop_name  = $ord_h_data["client_name"];
    $staff_name = $ord_h_data["staff_name"];
	$note_your  = str_replace("\n","",$ord_h_data["note_your"]);
	$hope_day   = $ord_h_data["hope_day"];
	$green_flg  = $ord_h_data["green_flg"];


	//希望納期
	if($hope_day != "" ){
	    $hope_date = explode("-",$hope_day);
	    $hope_date["0"] = $hope_date["0"] - 1989;
	}else{
	    $hope_date = array("　","　","　");
	}
}
...
//変数登録
$make_day = "平成　".$year."年　".$monty."月　".$day."日";
$shop_name = $shop_name;
