 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/11/25　0101　　　　 watanabe-k　 GETチェック追加
 * 　2006/11/25　0100　　　　 watanabe-k　 直送先名に空白チェック追加
...
if($direct_id != null){
    ...
    $def_fdata["form_direct_name2"]                   =    $data_list[14]; //直送先名
...
/****************************/
//直送先コード
$form->addElement("text","form_direct_cd","","size=\"4\" maxLength=\"4\" style=\"$g_form_style\"".$g_form_option."\"");
//直送先名
$form->addElement("text","form_direct_name","","size=\"44\" maxLength=\"25\"".$g_form_option."\"");
//直送先名2
$form->addElement("text","form_direct_name2","","size=\"44\" maxLength=\"25\"".$g_form_option."\"");
//略称
$form->addElement("text","form_direct_cname","","size=\"44\" maxLength=\"20\"".$g_form_option."\"");
...
//TEL
$form->addElement("text","form_tel","テキストフォーム","size=\"34\" maxLength=\"30\" style=\"$g_form_style\"".$g_form_option."\"");
//FAX
$form->addElement("text","form_fax","テキストフォーム","size=\"34\" maxLength=\"30\" style=\"$g_form_style\" ".$g_form_option."\"");
...
$text[] =& $form->createElement("text","form_client_name","テキストフォーム",'size="34" style="color : #000000; border : #ffffff 1px solid; background-color: #ffffff;" readonly');
...
//変更・一覧
$form->addElement("button","change_button","変更・一覧","onClick=\"javascript:Referer('1-1-218.php')\"");
//登録(ヘッダ)
...
$code_value .= Code_Value("t_client",$db_con,$where,1);
...
$form->addRule('form_direct_cname','略名にスペースのみの登録はできません。','no_sp_name');
...
if($_POST["entry_button"] == "登　録"){
    ...
    $direct_name2       = $_POST["form_direct_name2"];                  //直送先名2
    ...
    //◇請求先
    //・入力値チェック
    if(($client_cd1 != null && $client_name == null)
     || ($client_cd2 != null && $client_name == null)
     || ($client_cd1 != null && $client_cd2 != null && $client_name == null)){
    ...
    if($form->validate() && $err_flg == false){
        ...
        if($direct_id != null){
            ...
            $sql .= "   shop_id";
            $sql .= " )VALUES(";
            $sql .= "   (SELECT COALESCE(MAX(direct_id), 0)+1 FROM t_direct),";
            $sql .= "   '$direct_cd',";
            $sql .= "   '$direct_name',";
            $sql .= "   '$direct_name2',";
            $sql .= "   '$direct_cname',";
...
if($freeze_flg == true){
    ...
    //OK
    $form->addElement("button","comp_button","O 　K","onClick=\"location.href='./1-1-219.php'\"");

    $form->freeze();
}else{

    $form->addElement("link","intro_claim_link","","#","請求先","onClick=\"javascript:return Open_SubWin('../dialog/1-0-250.php',Array('form_client[form_client_cd1]','form_client[form_client_cd2]','form_client[form_client_name]'),500,450)\"");
    //登録
    ...
    //次へボタン    
    if($next_id != null){
        $form->addElement("button","next_button","次　へ","onClick=\"location.href='./1-1-219.php?direct_id=$next_id'\"");
    }else{  
        $form->addElement("button","next_button","次　へ","disabled");
    }
    //前へボタン
    if($back_id != null){
        $form->addElement("button","back_button","前　へ","onClick=\"location.href='./1-1-219.php?direct_id=$back_id'\"");
    }else{  
    ...
    if ($direct_id != null){
        //戻る
        $form->addElement("button","return_button","戻　る","onClick=\"location.href='./1-1-218.php'\"");
    }
