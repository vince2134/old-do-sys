...
//$form->addElement("hidden", "hdn_type");            //種別
$form->addElement("hidden", "hdn_state");           //状態
$form->addElement("hidden", "hdn_state_s");           //状態
...
//$set_data["hdn_type"]               = stripslashes($type);      //種別
...
/****************************/
$sql  = " SELECT";
$sql .= "     t_client.client_id,";                     // 0 得意先ID
$sql .= "     t_client.client_cd1,";                    // 1 得意先コード１
$sql .= "     t_client.client_cd2,";                    // 2 得意先コード２
$sql .= "     t_client.client_name,";                   // 3 得意先名
$sql .= "     t_client.client_cname,";                  // 4 得意先名（略称）
$sql .= "     t_area.area_name,";                       // 5 地区名
$sql .= "     t_client.tel,";                           // 6 TEL
$sql .= "     t_client.state,";                         // 7 状態
$sql .= "     t_client.type,";                          // 8 種別
$sql .= "     fc.client_name AS fc_name, ";             // 9 ショップ名
$sql .= "     t_client_claim.client_name AS claim_name, ";// 10 請求先
$sql .= "     t_sbtype.sbtype_id, ";                    // 11 業種ID
$sql .= "     fc.client_cd1, ";                         // 12 ショップコード１
$sql .= "     fc.client_cd2, ";                         // 13 ショップコード２
$sql .= "     t_client_claim.client_cd1,";              // 14 請求先１コード１
$sql .= "     t_client_claim.client_cd2,";              // 15 請求先１コード２
$sql .= "     t_client_claim2.client_cd1,";             // 16 請求先２コード１
$sql .= "     t_client_claim2.client_cd2,";             // 17 請求先２コード２
$sql .= "     t_client_claim2.client_name, ";           // 18 請求先２
$sql .= "     fc.state, ";                              // 19 状態（ショップ）
$sql .= "     t_rank.rank_name ";                       // 20 FC・取引先区分
if($output_type == 2){
$sql .= "   ,";
$sql .= "   t_lbtype.lbtype_name ||' '||t_sbtype.sbtype_name,";// 21  業種名
$sql .= "   t_inst.inst_name,";                         // 22  施設名
$sql .= "   t_bstruct.bstruct_name,";                   // 23  業態名
$sql .= "   t_client.client_read,";                     // 24  得意先名１（フリガナ）
$sql .= "   t_client.client_name2,";                    // 25  得意先名２
$sql .= "   t_client.client_read2,";                    // 26  得意先名２（フリガナ）
$sql .= "   t_client.client_cread,";                    // 27  略称（フリガナ）
...
$sql .= "   t_client_gr.client_gr_name,";               // 97 グループ名
$sql .= "   t_client.deliver_note, ";                   // 98 売上伝票コメント
...
$sql .= "   t_intro_ac.client_cd2,";                    // 104 照会口座コード２
$sql .= "   t_intro_ac.client_div, ";                    // 105 照会口座の取引区分
...
//print "<br>$i".$page_data[$i][96];
        $csv_page_data[$i][0] = ($page_data[$i][19] == "1") ? "取引中" : "解約・休止中"; //ショップ 状態
        $csv_page_data[$i][1] = $page_data[$i][12]."-".$page_data[$i][13];  //ショップコード
        $csv_page_data[$i][2] = $page_data[$i][9];  //ショップ名
        $csv_page_data[$i][3] = ($page_data[$i][7] == "1") ? "取引中" : "解約・休止中"; //得意先 状態
        $csv_page_data[$i][4] = $page_data[$i][97]; //グループ名
        $csv_page_data[$i][5] = (($page_data[$i][96] == 't')? "親":(($page_data[$i][96] == 'f')?"子":"独立"));    //親子区分
        $csv_page_data[$i][6] = $page_data[$i][5];      //地区
        $csv_page_data[$i][7] = $page_data[$i][21];     //業種
        $csv_page_data[$i][8] = $page_data[$i][22];     //施設
        $csv_page_data[$i][9] = $page_data[$i][23];     //業態
        $csv_page_data[$i][10] = $page_data[$i][1]."-".$page_data[$i][2];   //得意先コード
        $csv_page_data[$i][11] = $page_data[$i][3];     //得意先名１
        $csv_page_data[$i][12] = $page_data[$i][25];    //得意先名２
        $csv_page_data[$i][13] = $page_data[$i][24];    //得意先名１（フリガナ）
        $csv_page_data[$i][14] = $page_data[$i][26];    //得意先名２（フリガナ）
        $csv_page_data[$i][15] = $page_data[$i][4];     //略称
        $csv_page_data[$i][16] = $page_data[$i][27];    //略称（フリガナ）
        $csv_page_data[$i][17] = $page_data[$i][28];    //敬称
        $csv_page_data[$i][18] = $page_data[$i][29];    //代表者氏名
        $csv_page_data[$i][19] = $page_data[$i][30];    //代表者役職
        $csv_page_data[$i][20] = $page_data[$i][31]."-".$page_data[$i][32]; //郵便番号１
        $csv_page_data[$i][21] = $page_data[$i][33];    //住所１
        $csv_page_data[$i][22] = $page_data[$i][34];    //住所２
        $csv_page_data[$i][23] = $page_data[$i][35];    //住所３
        $csv_page_data[$i][24] = $page_data[$i][36];    //住所（フリガナ）
        $csv_page_data[$i][25] = $page_data[$i][6];     //TEL
        $csv_page_data[$i][26] = $page_data[$i][37];    //FAX
        $csv_page_data[$i][27] = $page_data[$i][38];    //創業日
        $csv_page_data[$i][28] = $page_data[$i][39];    //担当者EMAIL
        $csv_page_data[$i][29] = $page_data[$i][40];    //親会社名
        $csv_page_data[$i][30] = $page_data[$i][41];    //親会社TEL
        $csv_page_data[$i][31] = $page_data[$i][42];    //親会社住所
        $csv_page_data[$i][32] = $page_data[$i][43];    //資本金
        $csv_page_data[$i][33] = $page_data[$i][44];    //親会社創業日
        $csv_page_data[$i][34] = $page_data[$i][45];    //親会社代表者氏名
        $csv_page_data[$i][35] = $page_data[$i][46];    //URL
        $csv_page_data[$i][36] = $page_data[$i][47];    //担当１部署
        $csv_page_data[$i][37] = $page_data[$i][50];    //担当１役職
        $csv_page_data[$i][38] = $page_data[$i][53];    //担当１氏名
        $csv_page_data[$i][39] = $page_data[$i][48];    //担当２部署
        $csv_page_data[$i][40] = $page_data[$i][51];    //担当２役職
        $csv_page_data[$i][41] = $page_data[$i][54];    //担当２氏名
        $csv_page_data[$i][42] = $page_data[$i][49];    //担当３部署
        $csv_page_data[$i][43] = $page_data[$i][52];    //担当３役職
        $csv_page_data[$i][44] = $page_data[$i][55];    //担当３氏名
        $csv_page_data[$i][45] = $page_data[$i][56];    //担当者備考
        $csv_page_data[$i][46] = ($page_data[$i][57]!=null || $page_data[$i][58]!=null)?$page_data[$i][57]."〜".$page_data[$i][58]:"";    //営業時間（午前）
        $csv_page_data[$i][47] =  ($page_data[$i][59]!=null || $page_data[$i][60]!=null)?$page_data[$i][59]."〜".$page_data[$i][60]:"";    //営業時間（午後）
        $csv_page_data[$i][48] = $page_data[$i][61];    //休日
        $csv_page_data[$i][49] = $page_data[$i][14]."-".$page_data[$i][15]; //請求先１コード
        $csv_page_data[$i][50] = $page_data[$i][10];    //請求先１
        $csv_page_data[$i][51] = ($page_data[$i][16]!=null)?$page_data[$i][16]."-".$page_data[$i][17]:null; //請求先２コード
        $csv_page_data[$i][52] = $page_data[$i][18];    //請求先２
        $csv_page_data[$i][53] = $page_data[$i][63];    //与信限度
        $csv_page_data[$i][54] = $page_data[$i][64];    //回収条件
        $csv_page_data[$i][55] = $page_data[$i][65];    //取引区分
        $csv_page_data[$i][56] = $page_data[$i][66];    //締日
        $csv_page_data[$i][57] = $page_data[$i][67]."の".$page_data[$i][68];    //集金日
        $csv_page_data[$i][58] = $page_data[$i][69];    //集金方法
        $csv_page_data[$i][59] = $page_data[$i][70];    //振込銀行口座
        $csv_page_data[$i][60] = $page_data[$i][71];    //振込名義１
        $csv_page_data[$i][61] = $page_data[$i][72];    //振込名義２
        $csv_page_data[$i][62] = $page_data[$i][73];    //銀行手数料負担区分
        $csv_page_data[$i][63] = $page_data[$i][74];    //契約年月日
        $csv_page_data[$i][64] = $page_data[$i][75];    //契約更新日
        $csv_page_data[$i][65] = $page_data[$i][77];    //契約期間
        $csv_page_data[$i][66] = $page_data[$i][76];    //契約終了日
        $csv_page_data[$i][67] = $page_data[$i][78];    //伝票発行
        $csv_page_data[$i][68] = $page_data[$i][79];    //売上伝票発行元
        $csv_page_data[$i][69] = $page_data[$i][80];    //売上伝票コメント効果
        $csv_page_data[$i][70] = $page_data[$i][98];    //売上伝票コメント
        $csv_page_data[$i][71] = $page_data[$i][81];    //請求書発行
        $csv_page_data[$i][72] = $page_data[$i][82];    //請求書送付
        $csv_page_data[$i][73] = $page_data[$i][83];    //請求書発行元
        $csv_page_data[$i][74] = $page_data[$i][84];    //請求書備考
        $csv_page_data[$i][75] = $page_data[$i][85];    //金額丸め区分
        $csv_page_data[$i][76] = $page_data[$i][86];    //消費税：課税単位
        $csv_page_data[$i][77] = $page_data[$i][87];    //消費税：端数区分
        $csv_page_data[$i][78] = $page_data[$i][88];    //消費税：課税区分
        $csv_page_data[$i][79] = $page_data[$i][89];    //設備情報等・その他
        if($page_data[$i][105] == "3"){
            $page_data[$i][99] = $page_data[$i][99]."-".$page_data[$i][104];
        }
        $csv_page_data[$i][80] = $page_data[$i][99];    //ご紹介口座コード
        $csv_page_data[$i][81] = $page_data[$i][100];   //ご紹介口座名
        $csv_page_data[$i][82] = $page_data[$i][101];   //お振込先口座名
        $csv_page_data[$i][83] = $page_data[$i][102];   //銀行/支店名
        $csv_page_data[$i][84] = $page_data[$i][103];   //口座番号
        $csv_page_data[$i][85] = $page_data[$i][90];    //担当支店
        $csv_page_data[$i][86] = $page_data[$i][91];    //現契約担当
        $csv_page_data[$i][87] = $page_data[$i][92];    //初期契約社員
        $csv_page_data[$i][88] = $page_data[$i][93];    //巡回開始日
        $csv_page_data[$i][89] = $page_data[$i][94];    //取引履歴
        $csv_page_data[$i][90] = $page_data[$i][95];    //重要事項
