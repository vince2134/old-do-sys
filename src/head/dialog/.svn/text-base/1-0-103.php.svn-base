...
// デバッグ用ボタン
$form->addElement("button", "form_test",  "デバッグ用", "onClick=\"location.href='$_SERVER[PHP_SELF]'\"");
...
$form->addElement("hidden", "hdn_client_cd11_s");   // 上６桁検索用 ショップコード1（開始）
$form->addElement("hidden", "hdn_client_cd11_e");   // 上６桁検索用 ショップコード1（終了）
$form->addElement("hidden", "hdn_client_cd12");     // 下４桁検索用 ショップコード1
$form->addElement("hidden", "hdn_client_cd22_s");   // 下４桁検索用 ショップコード2（開始）
$form->addElement("hidden", "hdn_client_cd22_e");   // 下４桁検索用 ショップコード2（終了）
$form->addElement("hidden", "hdn_clear_flg");       // クリアボタン押下フラグ

// エラーセット用
$form->addElement("text", "err_client_cd1");        // ショップコード1
...
if ($_POST["show_button"] == "表　示"){
    ...
    // 下4桁検索の場合
        ...
        // コード2（開始）＞コード2（終了）
        if (str_pad($_POST["form_client_cd22"]["s"], 6, "0", STR_PAD_RIGHT) >
            str_pad($_POST["form_client_cd22"]["e"], 6, "9", STR_PAD_RIGHT)
        ){
            $form->setElementError("err_no_msg", null);
        }
    ...
    if ($err_flg != true){
        ...
        $client_cd11_s                  = trim($_POST["form_client_cd11"]["s"]);    // 上６桁検索用 ショップコード1（開始）
        $client_cd11_e                  = trim($_POST["form_client_cd11"]["e"]);    // 上６桁検索用 ショップコード1（終了）
        $client_cd12                    = trim($_POST["form_client_cd12"]);         // 下４桁検索用 ショップコード1
        $client_cd22_s                  = trim($_POST["form_client_cd22"]["s"]);    // 下４桁検索用 ショップコード2（開始）
        $client_cd22_e                  = trim($_POST["form_client_cd22"]["e"]);    // 下４桁検索用 ショップコード2（終了）

        // POSTデータをhiddenへ代入
        $hdn_set["hdn_range"]           = $_POST["form_range"];                     // 表示件数
        $hdn_set["hdn_method"]          = $_POST["form_method"];                    // 検索方法
        $hdn_set["hdn_client_cd11_s"]   = trim($_POST["form_client_cd11"]["s"]);    // 上６桁検索用 ショップコード1（開始）
        $hdn_set["hdn_client_cd11_e"]   = trim($_POST["form_client_cd11"]["e"]);    // 上６桁検索用 ショップコード1（終了）
        $hdn_set["hdn_client_cd12"]     = trim($_POST["form_client_cd12"]);         // 下４桁検索用 ショップコード1
        $hdn_set["hdn_client_cd22_s"]   = trim($_POST["form_client_cd22"]["s"]);    // 下４桁検索用 ショップコード2（開始）
        $hdn_set["hdn_client_cd22_e"]   = trim($_POST["form_client_cd22"]["e"]);    // 下４桁検索用 ショップコード2（終了）
...
if ($_POST != null && $_POST["show_button"] != "表　示"){
    ...
    $client_cd11_s                      = $_POST["hdn_client_cd11_s"];              // 上６桁検索用 ショップコード1（開始）
    $client_cd11_e                      = $_POST["hdn_client_cd11_e"];              // 上６桁検索用 ショップコード1（終了）
    $client_cd12                        = $_POST["hdn_client_cd12"];                // 下４桁検索用 ショップコード1
    $client_cd22_s                      = $_POST["hdn_client_cd22_s"];              // 下４桁検索用 ショップコード2（開始）
    $client_cd22_e                      = $_POST["hdn_client_cd22_e"];              // 下４桁検索用 ショップコード2（終了）
...
if ($post_flg == true){
    ...
    if ($method == "1"){
        ...
        // 終了番号が999999を超える場合は指定された終了コードにする
...
if ($post_flg == true){
    ...
    $html_l .= "        <td class=\"Title_Purple\">ショップコード</td>\n";
    if ($method != "1"){
        $html_l .= "        <td class=\"Title_Purple\">ショップ名</td>\n";
