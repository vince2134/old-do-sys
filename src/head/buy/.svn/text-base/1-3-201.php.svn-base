 ...
 *      ・支払い処理を追加
 *    2006/09/11 (watanabe-k)
 *      ・消費税額がおかしいバグの修正
 *    2006/09/14 (watanabe-k)
 *      ・支払日がおかいし。
 *      ・変更された場合は支払テーブルを一度消す
 *    2006/09/16 (kaji)
 *      ・割賦仕入をデフォルト2回に
 ...
 *      ・権限チェック修正
 *    2006/09/16 (watanabe-k)
 *      ・日付チェック追加
 *    2006/10/05 (watanabe-k)
 *      ・発注番号をブランクにして表示ボタンを押下した場合に値がクリアされないバグの修正
 *      ・存在する商品を入力後、存在しない商品を入力すると入数がクリアされなバグの修正
 *      ・仕入単価の小数点以下を入力せずに登録確認を実施するとえらメッセージが表示されるバグの修正
 *      ・仕入照会から変更のステータスで遷移してきた場合に品名変更不可商品の品名が変更できるバグの修正 
 *      ・仕入照会から変更のステータスで遷移してきた場合に発注番号を選択できてしまうバグの修正
 *      ・仕入確認画面に表示される金額と仕入編集画面に表示される金額が異なるバグの修正
 *      ・仕入日、入荷日に００月と存在しない月を入力すると、XXXに月事項審美以前の日付は登録できません。
 *       というエラーメッセージが表示されるバグの修正
 *      ・発注数より多くしいれることができてしまうバグの修正
 *      ・発注に対して仕入数を変更しても再度発注残が表示されないバグの修正
 *      ・発注残より少ない仕入数に変更しても、発注残一覧に対照データが表示されないバグの修正
 *      ・入力フォームが選択されてもバックカラーが黄色にならないバグの修正
 *   2006/10/12(watanabe-k)
 *      ・仕入先名略称を登録するように変更
 ...
 *   2006/10/13  06-013        watanabe-k  仕入倉庫を選択しても現在個数が変わらないバグの修正
 *   2006/10/18  06-018        watanabe-k  不正な商品コードを入力してもエラーメッセージが表示されないバグの修正 
 *   2006/10/18  06-037        watanabe-k  URL操作でSQLエラーが表示されるバグの修正 
 *   2006/10/19  06-042        watanabe-k  商品コードを不正入力し、フォーカスを異動しないまま発注確認画面へボタンをクリックすると、確認画面に遷移してしまう。
 *   2006/10/19  06-045        watanabe-k  仕入先コードを不正入力し、フォーカスを異動しないまま発注確認画面へボタンをクリックすると、確認画面に遷移してしまう。
 *   2006/11/11  06-069        watanabe-k  GETの値に日次更新済みのIDを指定した場合変更できてしまうバグの修正
 *   2006/11/11  06-070        watanabe-k  長い商品名を表示できるように修正
 *   2006/11/13  06-079        watanabe-k  仕入済の発注を変更できてしまうバグの修正
 *   2006/11/13  06-109        watanabe-k  ロット数に文字列を入力するとNaNと表示されるバグの修正
 *   2006/11/13  06-111        watanabe-k  強制完了している発注に対して仕入れを完了させることができるバグの修正
 *   2006/11/13  06-124        watanabe-k  ロット数が半角入力になっていないバグの修正
 *   2006/11/13  06-125        watanabe-k  確認画面でメッセージが表示されているバグの修正
 *   2006/12/01  scl_0044      watanabe-k  当月御買上額で金額が合わないバグの修正
 *   2006/01/17                watanabe-k  完了後にブラウザの戻るボタンを使用し、戻った後再度登録すると発注数を超えていますというエラーが表示されるので、
                                           完了画面へ遷移するように修正
 *   2007/01/25                watanabe-k  ボタンの色を変更
 *   2007/02/28                  morita-d  商品名は正式名称を表示するように変更 
 *   2007/03/09  要望9-1       kajioka-h   入荷日を変更すると仕入日が変わるように変更
 *   2007/03/13                watanabe-k  取引区分に指定した仕入先のものを表示するように変更
 *   2007/03/13                watanabe-k  商品複数選択時のメッセージを具体的に表示するように修正
 *   2007/05/18                watanabe-k  直送先のプルダウンを当幅で表示
 *   
 */
...
//    $sql .= "   t_client.client_name,\n";
    ...
    $set_data["form_client"]["cd"]              = $buy_h_data["client_cd1"];    //仕入先CD
//    $set_data["form_client"]["name"]            = $buy_h_data["client_name"];   //仕入先名
...
//    $sql .= "   t_goods_info.in_num\n";
    ...
    for($i = 0; $i < $num; $i++){
        ...
        if($buy_d_data["buy_num"]%$buy_d_data["in_num"] == 0 && $buy_d_data["in_num"]!=null){
...
//        $set_data["hdn_tax_france"]                 = $get_client_data["tax_franct"];   //丸め区分
        $set_data["hdn_tax_franct"]                 = $get_client_data["tax_franct"];   //丸め区分
        $set_data["hdn_coax"]                       = $get_client_data["coax"];         //端数区分
        $set_data["form_trade"]                     = $get_client_data["trade_id"];     //取引区分
...
//        $sql .= "   t_client.client_name,\n";
        ...
        $set_data["form_client"]["cd"]          = $get_ord_h_data["client_cd1"];    //仕入先CD
...
$update_form[1] = $form->addElement(
        ...
        onChange=\"javascript:Button_Submit('hdn_ware_select_flg','#','t')\""
);
...
$form->addElement("link","add_row_link","","#","追加","
    onClick=\"javascript:Button_Submit_1('add_row_flg', '#', 'true');\""
);
...
if($update_flg === false || $update_data_flg === false){
    //仕入先対象
    $form->addElement("button","form_client_button","仕入先",$g_button_color."onClick=\"javascript:location.href='./1-3-201.php'\"");
    //FC 対象
    $form->addElement("button","form_fc_button","Ｆ　Ｃ","onClick=\"javascript:location.href='./1-3-207.php';\"");
}
...
if($buy_button_flg == true || $buy_comp_flg == true){
    ...
    $client_cd                  = $_POST["form_client"]["cd"];              //仕入先CD
    ...
    //仕入日に３月以降の入力を禁止する
    //今後は仕入先に対しての入力がなくなるため、一時的な手段
    if(date('Y-m-d',mktime(0,0,0,3,1,2007)) 
        <= 
    str_pad($buy_day["y"],4,"0",STR_PAD_LEFT)."-".str_pad($buy_day["m"],2,"0",STR_PAD_LEFT)."-".str_pad($buy_day["d"],2,"0",STR_PAD_LEFT)){
        $form->setElementError("form_buy_day","３月以降の仕入はＦＣを仕入先として下さい。");
    }
    ...
    if($form->validate()){
        ...
        if($buy_comp_flg == true){
            ...
            //新規仕入入力の場合
                ...
                $sql .= "    client_name, ";
                $sql .= "    client_name2, ";
                $sql .= ($direct != null) ? " direct_name, " : null;
                $sql .= ($direct != null) ? " direct_name2, " : null;
                $sql .= "    ware_name, ";
                $sql .= "    c_staff_name, ";
                $sql .= ($order_staff != null) ? " oc_staff_name, " : null;
                $sql .= "    e_staff_name, ";
                $sql .= "    client_cname, ";
                $sql .= "    buy_div ";
                $sql .= ")VALUES(\n";
                ...
                $sql .= ($order_staff != null) ? " (SELECT staff_name FROM t_staff WHERE staff_id = $order_staff), " : null;
...
//                        }
                ...
                if($rollback_flg != true){
                    ...
                    header("Location:./1-3-205.php?buy_id=$buy_id&input_flg=true&buy_div=1");
                }
            }
...
if($select_flg == false || $order_flg == true){
    $style = "color : #000000;
              border : #ffffff 1px solid;
              background-color: #ffffff";
    $type = "readonly";
...
*/
        ...
        if($select_flg == false){
            ...
            $form_buy_price[$i][] =& $form->createElement(
                ...
                onKeyup=\"Mult('hdn_goods_id[$i]','form_buy_num[$i]','form_buy_price[$i][i]','form_buy_price[$i][d]','form_buy_amount[$i]','$coax');\"
                style=\"text-align: left; $g_form_style \" $g_form_option
            "
            );
        }
...
/*
        ...
        if($select_flg === true && $order_flg != true && $freeze_flg != true){
            $html .=    "（";
...
if($select_flg == true && $freeze_flg !== true){
    ...
    if($order_flg != true){
        $form->addElement("button","form_add_row_button", "追　加", 
            "onClick=\"javascript:Button_Submit('add_row_flg','./1-3-201.php#foot','true')\""
        );
        if($update_flg != true){
            //仕入先リンク
...
//        $select_flg = false;
    ...
    if($update_flg != true){    
        ...
        $form->addElement("button","form_show_button","表　示",
            "onClick=\"javascript:Button_Submit('hdn_show_button_flg','#','t')\""
        );
    }
