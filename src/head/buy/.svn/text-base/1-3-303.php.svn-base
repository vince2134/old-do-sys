...
/****************************/
/* 共通フォーム */
Search_Form_Payout_H($db_con, $form, $ary_form_list);
...
// ヘッダ部リンクボタン
$ary_h_btn_list = array("照会・変更" => $_SERVER["PHP_SELF"], "入　力" => "./1-3-302.php");
...
if ($post_flg == true && $err_flg != true){
    ...
    // 仕入先コード2
    if ($client_cd2 != null){
        $sql .= "AND t_payout_h.client_cd2 LIKE '$client_cd2%' \n";
...
if ($post_flg == true && $err_flg != true){
    ...
    $sql .= "       WHEN '2' THEN t_payout_h.client_cd1 \n";
    $sql .= "       WHEN '3' THEN t_payout_h.client_cd1 || '-' || t_payout_h.client_cd2 \n";
    $sql .= "   END \n";
    $sql .= "   AS client_cd1, ";
    $sql .= "   COALESCE(t_payout_d.pay_amount, 0), \n";
    $sql .= "   COALESCE(t_payout_d_48.pay_amount, 0) AS tax_amount, \n";
    $sql .= "   COALESCE(t_payout_d.pay_amount, 0) + COALESCE(t_payout_d_48.pay_amount, 0) AS sum_amount, \n";
    $sql .= "   t_payout_d.note, \n";
    ...
    $sql .= "FROM \n";
    $sql .= "   t_payout_h \n";
    $sql .= "   LEFT JOIN t_client ON t_payout_h.client_id = t_client.client_id \n";
    // 支払データ（手数料以外）
...
for($i = 0; $i < $row_count; $i++){
    $sum1 = bcadd($sum1,$page_data[$i][13]);                 //支払金額合計
    $sum2 = bcadd($sum2,$page_data[$i][14]);                 //手数料合計
    $sum3 = bcadd($sum3,$page_data[$i][15]);                 //合計金額
}
...
/****************************/
// モジュール個別検索テーブル用テンプレ１
$html_s_tp1  = "<br style=\"font-size: 4px;\">\n";
$html_s_tp1 .= "\n";
$html_s_tp1 .= "<table class=\"Table_Search\">\n";
$html_s_tp1 .= "    <col width=\" 80px\" style=\"font-weight: bold;\">\n";
$html_s_tp1 .= "    <col width=\"300px\">\n";
$html_s_tp1 .= "    <col width=\"100px\" style=\"font-weight: bold;\">\n";
$html_s_tp1 .= "    <col width=\"300px\">\n";
$html_s_tp1 .= "    <tr>\n";
// モジュール個別検索テーブル用テンプレ２
$html_s_tp2  = "    </tr>\n";
...
// 共通検索テーブル
$html_s  = Search_Table_Payout_H($form);
// モジュール個別検索テーブル１
$html_s .= $html_s_tp1;
