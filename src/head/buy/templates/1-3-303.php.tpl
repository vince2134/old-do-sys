...
<table class="List_Table" border="1" width="100%">
    ...
    {/if}
        ...
        {*支払番号*}
        <td align="center"><a href="1-3-302.php?pay_id={$item[0]}">{$item[1]}</a></td>
        {*入力日と支払日*}
        ...
        {*支払金額*}
        {if $item[13]<0}<td align="right"><font color="#ff0000">{$item[13]|number_format}</font></td>
        {else}<td align="right">{$item[13]|number_format}</td>{/if}
        {*手数料*}
        {if $item[14]<0}<td align="right"><font color="#ff0000">{$item[14]|number_format}</font></td>
        {else}<td align="right">{$item[14]|number_format}</td>{/if}
        {*合計金額*}
        {if $item[15]<0}<td align="right"><font color="#ff0000">{$item[15]|number_format}</font></td>
        {else}<td align="right">{$item[15]|number_format}</td>{/if}
        {*決済日/手形券面番号*}
        <td>{$item[20]}<br>{$item[21]}<br></td>
        {*備考*}
        <td>{$item[16]}</td>
        {*日次更新*}
        {if $item[17]=='t'}
        ...
        <td align="center"></td>
        {*削除*}
        <td align="center">{if $item[18] == null && $item[19] == null}{if $var.auth == "w"}<a href="#" onClick="Order_Delete('data_delete_flg','pay_h_id','{$item[0]}');">削除</a>{/if}{/if}</td>
        {/if}
    </tr>
    {/foreach}
    <tr class="Result3" height="30px">
