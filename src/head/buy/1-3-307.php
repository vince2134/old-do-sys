 ...
 * 　2006/11/27　0004　　　　　watanabe-k　集金日が「2ヶ月後の月末」の場合、当月支払に割賦金額が2ヶ月分上がってしまうバグの修正
 *   2006-12-09  ban_0110      suzuki      日付をゼロ埋め
 *   2006-12-14  scl_0080    watanabe-k    2ヶ月目以降の支払予定額が正しくないバグの修正  
 *   2007-01-09              watanabe-k    仕入先がFCになることを考慮する処理を追加
 *   2007-01-19              watanabe-k    未来の日付で支払い締めが行えるバグの修正
 *   2007-01-22              watanabe-k    2ヶ月目以降の支払残高額が正しくないバグの修正
 *   2007-01-22              watanabe-k    2月分の支払予定額が正しくないバグの修正
 *   2007-01-23              watanabe-k    一括消費税の処理を追加
 *   2007-06-16              watanabe-k    残高0の場合に間違ったデータが出来上がるバグの修正 
 *   2007-07-11                 fukuda      「支払締」を「仕入締」に変更
 *   2007-08-20              watanabe-k    取引が合った場合にも金額が０であれば、支払予定データを削除していたが、削除しないように修正
 *   2007-08-22              watanabe-k    割賦仕入の金額抽出を請求と同じ仕様に統一
 *   2007-11-16              watanabe-k    削除処理を追加し、全ての金額が０の場合という条件を追加した。
...
// 画面ヘッダー作成
// 支払予定一覧
$form->addElement("button", "1-3-307", "仕入締処理", "style=color:#ff0000 onClick=\"location.href='$_SERVER[PHP_SELF]'\"".$g_button_color);
// 入力
$form->addElement("button", "1-3-301", "支払予定一覧", "onClick=\"javascript:Referer('1-3-301.php')\"");
$page_title .= "　".$form->_elements[$form->_elementIndex["1-3-307"]]->toHtml();
$page_title .= "　".$form->_elements[$form->_elementIndex["1-3-301"]]->toHtml();
...
$data .= "         t_buy_h.shop_id = $2 \n";
$data .= " AND \n";
//期間で集計するのではなく、支払予定IDのNULLのものを集計する
//$data .= "         t_amortization.pay_day > $3 \n";
...
//		$future_flg = (date('Ymt') < $post_close_day_y.$post_close_day_m.$post_close_day_d)? true : false;
		...
		$future_flg = (date('Ymt') < $target_day)? true : false;
	}
		...
		if($duplicate_err != true && $renew_flg != true){
            ...
            if(count($buying_for_appoint) > 0){
				...
				for($i = 0; $i < count($buying_for_appoint); $i++){
					...
					//今回支払予定額
	//				$set_schedule_of_payment_this = (int)bcsub(bcadd(bcadd(bcsub($get_payment_array[$i][0][total_ca_balance_this], $get_payment_array[$i][total_installment_purchase],0),$buying_for_appoint[$i][total_split_pay_amount],0),$buying_for_appoint[$i][schedule_of_payment_this],0), $buying_for_appoint[$i][total_pay_amount],0);
                    ...
                    //一括消費税
                    $lump_tax_amount             = $result_amount[$i][lump_tax_amount];
...
//                            $sql = "DELETE FROM t_schedule_payment WHERE schedule_payment_id = ".$update_id[0]["max"].";";
						...
						//EXECUTE実行
						$sql = "EXECUTE update_t_schedule_payment (".$update_schedule_of_payment_this.", ".$update_pay_array.", ".$update_id[0][max].", '$today'); \n";
						$result = Db_Query($conn, $sql);
...
//				print "端数単位エラー";
        ...
        **** 2009-12-28 aoyama-n ****/
		...
		if($array[$i][tax_div] == '1'){
            ...
            $result[$i][lump_tax_amount] = ($tax_amount3[$i] - $result[$i][tax_amount]) * -1;
 ...
 * @version		1.0.1 (2007/01/22) watanabe-k 2月の月末計算が正しく動作しなかったため修正
 *
 */
