 ...
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 *  2006/11/13      03-082      ふ          割賦設定中に仕入伝票が変更された場合の対処
 *  2006/12/14      kaji-0199   watanbe-k   割賦入力中に伝票を削除された場合にクエリエラーが表示されるバグの修正
 *  2007/01/17      仕様変更    watanbe-k   分割回数を２〜６０に変更
...
// 分割設定ボタン
$form->addElement("button", "form_conf_button", "分割設定", "onClick=\"Button_Submit('hdn_division_submit','#','t', this);\" $disabled");
...
//支払日（毎月）
$sql = "SELECT payout_m, payout_d FROM t_client WHERE client_id = ".$h_data_list[0][13].";";
$result = Db_Query($db_con,$sql);
$payout_m = pg_fetch_result($result, 0, "payout_m");
$payout_d = pg_fetch_result($result, 0, "payout_d");
$def_fdata["form_pay_d"] = $payout_d;
$def_fdata["hdn_payout_m"] = $payout_m;
...
*/
    ...
    if (isset($_POST["add_button"])){
        ...
        if (pg_num_rows($res) == 0){
            ...
            header("Location:1-3-205.php?inst_err=true&buy_id=0&input_flg=true");
            exit;
        }
