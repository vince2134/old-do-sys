 ...
 *  日付            B票No.      担当者      内容
 *  -----------------------------------------------------------
 *  2006/11/11      06-059      ふ          リロードによる伝票削除防止のため、伝票削除時に伝票作成日時と照らし合わせる
 *  2006/11/11      06-088      ふ          仕入入力済の伝票を削除できないよう修正
 *  2006/12/06      ban_0029    suzuki      日付のゼロ埋め追加
 *  2006/12/06      ban_0030    suzuki      日付不正時にエラー出力
 *  2007/01/24      仕様変更    watanabe-k  ボタンの色変更
 *  2007/02/07                  watanabe-k  発信日を表示しないように修正
 *  2007/02/07                  watanabe-k  発注書発行を発注書印刷に変更
 *  2007-04-04                  fukuda      検索条件復元処理追加
 *  2007-05-07                  fukuda      ソート順を日付の昇順に変更
 *  2007/08/22                  kajioka-h   一覧テーブルのヘッダを発注日時→発注日、仕入先→発注先に変更
 *
 */
...
// ヘッダ部リンクボタン
$form->addElement("button", "104_button", "照会・変更", "$g_button_color onClick=\"javascript:location.href('".$_SERVER["PHP_SELF"]."');\"");
$form->addElement("button", "102_button", "入　力",     "onClick=\"javascript:location.href('1-3-102.php');\"");
$form->addElement("button", "106_button", "発注残一覧", "onClick=\"javascript:location.href('1-3-106.php');\"");
