<?php
/********************************/
//  ÊÑ¹¹ÍúÎò
//      ¼è°úÀè¥³¡¼¥É£²¤òÃê½Ð¤·¤Ê¤¤¤è¤¦¤ËÊÑ¹¹
//    2006/07/06 (kaji)
//      shop_gid¤ò¤Ê¤¯¤¹
/********************************/

/*
 * ÍúÎò¡§
 * ¡¡ÆüÉÕ¡¡¡¡¡¡¡¡BÉ¼No.¡¡¡¡¡¡¡¡Ã´Åö¼Ô¡¡¡¡¡¡ÆâÍÆ¡¡
 * ¡¡2006/11/11¡¡06-046¡¡¡¡¡¡¡¡watanabe-k¡¡GET¤Î¥Á¥§¥Ã¥¯ÄÉ²Ã
 * ¡¡2006/11/11¡¡06-052¡¡¡¡¡¡¡¡watanabe-k¡¡GET¤Î¥Á¥§¥Ã¥¯ÄÉ²Ã
 * ¡¡2007/02/05¡¡      ¡¡¡¡¡¡¡¡watanabe-k¡¡È¯¿®Æü¤òÉ½¼¨¤·¤Ê¤¤¤è¤¦¤Ë½¤Àµ
 * ¡¡2009/09/18¡¡      ¡¡¡¡¡¡¡¡aoyama-n    ÃÍ°ú¾¦ÉÊ¤Î¾ì¹ç¤ÏÀÖ»ú¤ÇÉ½¼¨
 * ¡¡2009/09/18¡¡      ¡¡¡¡¡¡¡¡aoyama-n    ¾¦ÉÊ¤Î¥½¡¼¥È½ç¤òline¤Ë½¤Àµ
 *
 */



$page_title = "È¯Ãí¾È²ñ";

//´Ä¶­ÀßÄê¥Õ¥¡¥¤¥ë
require_once("ENV_local.php");

//HTML_QuickForm¤òºîÀ®
$form =& new HTML_QuickForm("dateForm", "POST", $_SERVER["PHP_SELF"]);

//DBÀÜÂ³
$db_con = Db_Connect();

// ¸¢¸Â¥Á¥§¥Ã¥¯
$auth       = Auth_Check($db_con);


/****************************/
//¥¯¥¨¥ê¼Â¹Ô´Ø¿ô
/****************************/
//(¥¯¥¨¥ê¡¼·ë²Ì¡¦¶¯À©´°Î»¥Õ¥é¥°¡¦½èÍý¾õ¶·)
function Get_Ord_Data($result,$finish_flg,$ord_stat,$discount_flg){
    $result_count = pg_numrows($result);   //Í×ÁÇ¿ô
    $day_flg      = false;                 //Ï¢·ë¥Õ¥é¥°
    $num          = 1;                     //Ï¢·ë¹Ô¿ô

    for($i = 0; $i < $result_count; $i++){
        $ord_data[$i] = @pg_fetch_array ($result, $i, PGSQL_NUM);
        //Ï¢·ë¤·¤¿¹Ô¤ÏÈóÉ½¼¨
        if($day_flg == true){
            $row[$i-1] = NULL;
            $day_flg = false;
        }
        //¹Ô¤Î¿§»ØÄê
        //aoyama-n 2009-09-18
        if($discount_flg[$i] === 't'){
            $font_color = 'color: red; ';
        }else{
            $font_color = 'color: #555555; ';
        }
        #$row[$i][0] = "<tr class=\"Result1\">";
        $row[$i][0] = "<tr class=\"Result1\" style=\"$font_color\">";
        for($j=0;$j<count($ord_data[$i]);$j++){
            //È¯Ãí¿ô
            if($j==2){
                $ord_data[$i][$j] = number_format($ord_data[$i][$j]);
                $ord_array = $ord_data[$i][$j];
            //»ÅÆþÃ±²Á
            }else if($j==3){
                $ord_data[$i][$j] = number_format($ord_data[$i][$j],2);
                $ord_array = $ord_data[$i][$j];
            //»ÅÆþ¶â³Û
            }else if($j==4){
                $ord_data[$i][$j] = number_format($ord_data[$i][$j]);
                $ord_array = $ord_data[$i][$j];
/*
            //ËÜÉô½Ð²ÙÍ½ÄêÆü
            }else if($j==5 && $ord_stat=='2'){
                //¾¦ÉÊ¥³¡¼¥É¤¬Æ±¤¸´Ö¡¢ËÜÉô½Ð²ÙÍ½ÄêÆü¤òÏ¢·ë¤¹¤ë
                if($row[$i-$num][1]==$ord_data[$i][0]){
                    //Á°¤Î¹Ô¤ÈÆ±¤¸¾ì¹çÏ¢·ë¤¹¤ë
                    $ord_data[$i][$j] = htmlspecialchars($ord_data[$i][$j]);
                    $row[$i-$num][$j+1] = $row[$i-$num][$j+1]."<br>".$ord_data[$i][$j];
                    //Ï¢·ë¤·¤¿¹Ô¤ÏÉ½¼¨¤·¤Ê¤¤
                    $day_flg = true;
                    $num++;
                }else{
                    $num = 1;
                    $ord_array = $ord_data[$i][$j];
                }
*/
            //È¯Ãí´°Î»ÍýÍ³
            }else if($j==7 || $j==8){
                if($ord_data[$i][$j] == NULL){
                    $ord_array = "¡¡";
                }else{
                    $ord_array = $ord_data[$i][$j];
                }
            }else{
                $ord_array = $ord_data[$i][$j];
            }

            if($day_flg == false){
                $row[$i][$j+1] = htmlspecialchars($ord_array);
            }
        }
    }
    //ºÇ½ª¹Ô¤¬Ï¢·ë¤·¤¿¹Ô¤«
    if($day_flg == true){
        $row[$i-1] = NULL;
    }

    //NULL¤Î¹Ô¤ÏÇÛÎó¤ËÆþ¤ì¤Ê¤¤
    for($i = 0; $i < count($row); $i++){
        if($row[$i]!=null){
            $row_data[] = $row[$i];
        }
    }

    return $row_data;
}

/****************************/
//¥Ç¡¼¥¿¹àÌÜºîÀ®´Ø¿ô
/****************************/
//(¶¯À©´°Î»¥Õ¥é¥°¡¦½èÍý¾õ¶·)
function Get_Ord_Item($finish_flg,$ord_stat){
    //¶¯À©¥Õ¥é¥°¤¬true¤«
    if($finish_flg == 't'){
        //È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¤«
        if($ord_stat == '2'){
            //­¡¶¯À©´°Î»¥Õ¥é¥°¡á£ô ¡õ È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¡¡¤Î¾ì¹ç
//            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û","ËÜÉô½Ð²ÙÍ½ÄêÆü","Æþ²Ù¿ô","È¯Ãí»Ä","È¯Ãí´°Î»ÍýÍ³");
            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û","Æþ²Ù¿ô","È¯Ãí»Ä","È¯Ãí´°Î»ÍýÍ³");
        }else{
            //­¢¶¯À©´°Î»¥Õ¥é¥°¡á£ô¡¡¤Î¾ì¹ç
            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û","Æþ²Ù¿ô","È¯Ãí»Ä","È¯Ãí´°Î»ÍýÍ³");
        }
    }else{
        //È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¤«
        if($ord_stat == '2'){
            //­£¶¯À©´°Î»¥Õ¥é¥°¡á£æ ¡õ È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¡¡¤Î¾ì¹ç
//            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û","ËÜÉô½Ð²ÙÍ½ÄêÆü");
            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û");
        }else{
            //­¤¶¯À©´°Î»¥Õ¥é¥°¡á£æ¡¡¤Î¾ì¹ç
            $row_item[] = array("No.","¾¦ÉÊ¥³¡¼¥É<br>¾¦ÉÊÌ¾","È¯Ãí¿ô","»ÅÆþÃ±²Á","»ÅÆþ¶â³Û");
        }
    }

    return $row_item;
}

/****************************/
//È¯Ãí½ñÈ¯¹Ô½èÍý
/****************************/
$order_sheet  = " function Order_Sheet(hidden1,ord_id){\n";
$order_sheet .= "   var id = ord_id;\n";
$order_sheet .= "   var hdn1 = hidden1;\n";
$order_sheet .= "   window.open('../../head/buy/1-3-105.php?ord_id='+id,'_blank','');\n";
$order_sheet .= "   document.dateForm.elements[hdn1].value = ord_id;\n";
$order_sheet .= "   //Æ±¤¸¥¦¥£¥ó¥É¥¦¤ÇÁ«°Ü¤¹¤ë\n";
$order_sheet .= "   document.dateForm.target=\"_self\";\n";
$order_sheet .= "   //¼«²èÌÌ¤ËÁ«°Ü¤¹¤ë\n";
$order_sheet .= "   document.dateForm.action='#';\n";
$order_sheet .= "   //POST¾ðÊó¤òÁ÷¿®¤¹¤ë\n";
$order_sheet .= "   document.dateForm.submit();\n";
$order_sheet .= "   return true;\n";
$order_sheet .= "}\n";

/****************************/
//³°ÉôÊÑ¿ô¼èÆÀ
/****************************/
$staff_id     = $_SESSION[staff_id];
$shop_id      = $_SESSION[client_id];
//$shop_gid     = $_SESSION[shop_gid];
$ord_id       = $_GET["ord_id"];             //È¯ÃíID
Get_Id_Check3($ord_id);
Get_Id_Check2($ord_id);
$ord_flg      = $_GET["ord_flg"];            //È¯Ãí¾È²ñ¼±ÊÌ¥Õ¥é¥°
$output_flg   = $_GET["output_flg"];         //È¯Ãí½ñ½ÐÎÏ¥Õ¥é¥°

/****************************/
//ÉôÉÊÄêµÁ
/****************************/
//È¯¿®Æü
$form->addElement(
    "text","form_send_date","",
    "size=\"25\" maxLength=\"18\" 
    style=\"color : #585858; 
    border : #FFFFFF 1px solid; 
    background-color: #FFFFFF; 
    text-align: right\" readonly'"
);

//È¯ÃíÆü
$text="";
$text[] =& $form->createElement("static","y","","-");
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("static","m","","-");
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("static","d","","-");
$form->addGroup( $text,"form_ord_time","form_ord_time");


//Æþ²ÙÍ½ÄêÆü
$text="";
$text[] =& $form->createElement("static","y","","-");
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("static","m","","-");
$text[] =& $form->createElement("static","","","-");
$text[] =& $form->createElement("static","d","","-");
$form->addGroup( $text,"form_arrival_day","form_arrival_day");

//´õË¾Ç¼´ü
$text="";
$text[] =& $form->createElement("static","y","","");
$text[] =& $form->createElement("static","m","","");
$text[] =& $form->createElement("static","d","","");
$form->addGroup( $text,"form_hope_day","form_hope_day");
    
//È¯ÃíÈÖ¹æ
$form->addElement("static","form_ord_no","","");

//»ÅÆþÀèÌ¾
$form_client[] =& $form->createElement("static","cd1","","");
$form_client[] =& $form->createElement("static","cd2","","");
$form_client[] =& $form->createElement("static","",""," ");
$form_client[] =& $form->createElement("static","name","","");
$form->addGroup( $form_client, "form_client", "");

//Ä¾Á÷ÀèÌ¾
$form->addElement("static","form_direct_name","","");
//»ÅÆþÁÒ¸Ë
$form->addElement("static","form_ware_name","","");
//¼è°ú¶èÊ¬
$form->addElement("static","form_trade_ord","","");
//Ã´Åö¼Ô
$form->addElement("static","form_c_staff_name","","");
//ÄÌ¿®Íó¡Ê»ÅÆþÀè°¸¡Ë
$form->addElement("static","form_note_my","","");
//ÄÌ¿®Íó¡ÊËÜÉô°¸¡Ë
$form->addElement("static","form_note_your","","");
//ÂèÆóÄÌ¿®Íó¡ÊËÜÉô°¸¡Ë
$form->addElement("static","form_note_your2","","");

//Çä¾å¶â³Û¹ç·×
$form->addElement(
    "text","form_buy_total","",
    "size=\"25\" maxLength=\"18\" 
    style=\"color : #585858; 
    border : #FFFFFF 1px solid; 
    background-color: #FFFFFF; 
    text-align: right\" readonly'"
);

//¾ÃÈñÀÇ³Û(¹ç·×)
$form->addElement(
        "text","form_buy_tax","",
        "size=\"25\" maxLength=\"18\" 
        style=\"color : #585858; 
        border : #FFFFFF 1px solid; 
        background-color: #FFFFFF; 
        text-align: right\" 
        readonly"
);

//Çä¾å¶â³Û¡ÊÀÇ¹þ¹ç·×)
$form->addElement(
        "text","form_buy_money","",
        "size=\"25\" maxLength=\"18\" 
        style=\"color : #585858; 
        border : #FFFFFF 1px solid; 
        background-color: #FFFFFF; 
        text-align: right\" 
        readonly"
);

//È¯Ãí½ñ½ÐÎÏ
/*
$first_list  = " SELECT ";
$first_list .= " t_client.head_flg ";
$first_list .= " FROM ";
$first_list .= " t_order_h ";
$first_list .= " INNER JOIN ";
$first_list .= " t_client ";
$first_list .= " ON t_order_h.client_id = t_client.client_id ";
$first_list .= " WHERE ";
$first_list .= " t_order_h.ord_id = $ord_id;";
$result = Db_Query($db_con, $first_list);
$head_flg = pg_fetch_result($result,0,0);
*/
//onLoad½èÍý
//È¯Ãí½ñ½ÐÎÏ¥Ü¥¿¥ó¤¬²¡¤µ¤ì¤ÆÁ«°Ü¤·¤Æ¤­¤¿¾ì¹ç¤Ë½ÐÎÏ
if($output_flg  == "true"){
    $load = "onLoad=\"javascript:Order_Sheet('order_sheet_id',$ord_id);\"";
}

//È¯Ãí½ñÈ¯¹ÔID
$form->addElement("hidden", "order_sheet_id");

//Á«°Ü¸µ¥Á¥§¥Ã¥¯
if(isset($_GET["output_flg"])){
    //OK¥Ü¥¿¥ó
    $form->addElement("button", "ok_button", "£Ï¡¡£Ë", "onClick=\"location.href='".Make_Rtn_Page("ord")."'\"");
    if($_GET[output_flg] != "delete" && $_GET[output_flg] != "finish"){
    //Ìá¤ë
        $form->addElement("button","return_button","Ìá¡¡¤ë","onClick=\"location.href='1-3-102.php?ord_id=$ord_id'\"");
    }

    $freeze_flg = true;    //È¯Ãí´°Î»¥á¥Ã¥»¡¼¥¸É½¼¨¥Õ¥é¥°
}else{
    //È¯Ãí¾È²ñ²èÌÌ
    if($ord_flg == true){
        //Ìá¤ë
//        $form->addElement("button","return_button","Ìá¡¡¤ë","onClick=\"location.href='1-3-104.php')\"");
        $form->addElement("button","return_button","Ìá¡¡¤ë","onClick=\"javascript:history.back()\"");
    }else{
        //Ìá¤ë
        $form->addElement("button","return_button","Ìá¡¡¤ë","onClick=\"javascript:history.back()\"");
    }
}

/****************************/
//È¯Ãí½ñÈ¯¹Ô¥Ü¥¿¥ó²¡²¼½èÍý
/****************************/
if($_POST["order_sheet_id"]!=NULL){

    Db_Query($db_con, "BEGIN");

    $flg_update  = " UPDATE ";
    $flg_update .= "    t_order_h ";
    $flg_update .= "    SET ";
    $flg_update .= "    ord_sheet_flg = 't' ";
    $flg_update .= "    where ";
    $flg_update .= "    ord_id = ".$_POST["order_sheet_id"];
    $flg_update .= ";";

    //³ºÅö¥Ç¡¼¥¿·ï¿ô
    $result = @Db_Query($db_con, $flg_update);
    if($result == false){
        Db_Query($db_con, "ROLLBACK");
        exit;
    }
    Db_Query($db_con, "COMMIT");

    $load = NULL;
}

if($_GET[output_flg] != "delete" && $_GET[output_flg] != "finish"){

    /****************************/
    //È¯Ãí¥Ø¥Ã¥À¡¼Ãê½ÐÈ½Äê½èÍý
    /****************************/
    $sql  = "SELECT ";
    $sql .= "    ord_stat,";
    $sql .= "    ps_stat,";
    $sql .= "    finish_flg ";
    $sql .= "FROM ";
    $sql .= "    t_order_h ";
    $sql .= "WHERE ";
    $sql .= "    t_order_h.ord_id = $ord_id";
    $sql .= "    AND";
    $sql .= "    t_order_h.shop_id = $shop_id";
    $sql .= ";";

    $result = Db_Query($db_con,$sql);
    //GET¥Ç¡¼¥¿È½Äê
    Get_Id_Check($result);

    //È¯Ãí¾õ¶·¼èÆÀ
    $ord_stat = pg_fetch_result($result,0,0);
    //½èÍý¾õ¶·¼èÆÀ
    $ps_stat = pg_fetch_result($result,0,1);
    //¶¯À©´°Î»¥Õ¥é¥°¼èÆÀ
    $finish_flg = pg_fetch_result($result,0,2);

    //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
    if($ps_stat == '4'){
        /****************************/
        //È¯Ãí¥Ø¥Ã¥ÀÃê½ÐSQL¡Ê½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¡Ë
        /****************************/
        $sql  = "SELECT ";
        $sql .= "    t_order_h.ord_no,";
        $sql .= "    to_char(t_order_h.ord_time, 'yyyy-mm-dd'),";                        
        $sql .= "    t_order_h.hope_day,";
        $sql .= "    t_order_h.arrival_day,";
        $sql .= "    t_order_h.green_flg,";
        $sql .= "    t_order_h.trans_name,";
        $sql .= "    t_order_h.client_cd1,";
//      $sql .= "    t_order_h.client_cd2,";
        $sql .= "    t_order_h.client_name,";
        $sql .= "    t_order_h.direct_name,";
        $sql .= "    t_order_h.ware_name,";
        $sql .= "    CASE t_order_h.trade_id";            
        $sql .= "        WHEN '21' THEN '³Ý»ÅÆþ'";
        $sql .= "        WHEN '71' THEN '¸½¶â»ÅÆþ'";
        $sql .= "    END,";
        $sql .= "    t_order_h.c_staff_name,";
        $sql .= "    t_order_h.note_my,";
        $sql .= "    t_order_h.note_your, ";
        $sql .= "    t_order_h.net_amount, ";
        $sql .= "    t_order_h.tax_amount, ";
        $sql .= "    to_char(t_order_h.send_date, 'yyyy-mm-dd hh24:mi') AS send_date, \n";
        $sql .= "   t_order_h.note_your2 \n";   // ÂèÆóÄÌ¿®Íó¡Ê»ÅÆþÀè°¸¡Ë
        $sql .= " FROM ";
        $sql .= "    t_order_h ";
        $sql .= "WHERE ";
        $sql .= "    t_order_h.ord_id = $ord_id;";
    }else{
        /****************************/
        //È¯Ãí¥Ø¥Ã¥À¡¼Ãê½ÐSQL
        /****************************/
        $sql  = "SELECT ";
        $sql .= "    t_order_h.ord_no,";
        $sql .= "    to_char(t_order_h.ord_time, 'yyyy-mm-dd'),";
        $sql .= "    t_order_h.hope_day,";
        $sql .= "    t_order_h.arrival_day,";
        $sql .= "    t_order_h.green_flg,";
//      $sql .= "    t_trans.trans_name,";
        $sql .= "    t_order_h.trans_name,";
//      $sql .= "    t_client.client_cd1,";
        $sql .= "    t_order_h.client_cd1,";
//      $sql .= "    t_client.client_cd2,";
//      $sql .= "    t_client.client_name,";
        $sql .= "    t_order_h.client_name,";
//      $sql .= "    t_direct.direct_name,";
        $sql .= "    t_order_h.direct_name,";
//      $sql .= "    t_ware.ware_name,";
        $sql .= "    t_order_h.ware_name,";
        $sql .= "    CASE t_order_h.trade_id";
        $sql .= "        WHEN '21' THEN '³Ý»ÅÆþ'";
        $sql .= "        WHEN '71' THEN '¸½¶â»ÅÆþ'";
        $sql .= "    END,";
//      $sql .= "    t_staff.staff_name,";
        $sql .= "    t_order_h.c_staff_name,";
        $sql .= "    t_order_h.note_my,";
        $sql .= "    t_order_h.note_your,";
        $sql .= "    t_order_h.net_amount, ";
        $sql .= "    t_order_h.tax_amount, ";
        $sql .= "    to_char(t_order_h.send_date, 'yyyy-mm-dd hh24:mi') AS send_date, ";
        $sql .= "   t_order_h.note_your2 \n";   // ÂèÆóÄÌ¿®Íó¡Ê»ÅÆþÀè°¸¡Ë
        $sql .= " FROM ";
        $sql .= "    t_order_h ";

        $sql .= "    LEFT JOIN ";
        $sql .= "    t_trans  ";
        $sql .= "    ON t_order_h.trans_id   = t_trans.trans_id ";

        $sql .= "    LEFT JOIN ";
        $sql .= "    t_direct ";
        $sql .= "    ON t_order_h.direct_id  = t_direct.direct_id ";

        $sql .= "    INNER JOIN t_client ON t_order_h.client_id  = t_client.client_id ";
        $sql .= "    INNER JOIN t_ware   ON t_order_h.ware_id    = t_ware.ware_id ";
        $sql .= "    INNER JOIN t_staff  ON t_order_h.c_staff_id = t_staff.staff_id ";
        $sql .= "WHERE ";
        $sql .= "    t_order_h.ord_id = $ord_id;";
    }
    $result = Db_Query($db_con,$sql);
    $h_data_list = Get_Data($result);

    //¶¯À©´°Î»¥Õ¥é¥°¤¬true¤«
    if($finish_flg == 't'){
        //È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¤«
        if($ord_stat == '2'){
            /****************************/
            //È¯Ãí¥Ç¡¼¥¿+(½Ð²ÙÍ½ÄêÆü¡¦Æþ²Ù¿ô¡¦È¯Ãí»Ä¡¦È¯Ãí´°Î»ÍýÍ³) Ãê½ÐSQL
            //­¡¶¯À©´°Î»¥Õ¥é¥°¡á£ô ¡õ È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¡¡¤Î¾ì¹ç
            /****************************/
            $data_sql  = "SELECT ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                $data_sql .= "    t_order_d.goods_cd,";
            }else{
//            $data_sql .= "    t_goods.goods_cd,";
                $data_sql .= "    t_order_d.goods_cd,";
            }
            $data_sql .= "    t_order_d.goods_name,";
            $data_sql .= "    t_order_d.num,";            
            $data_sql .= "    t_order_d.buy_price,";
            $data_sql .= "    t_order_d.buy_amount,";
            $data_sql .= "    t_aorder.arrival_day,";
            $data_sql .= "    COALESCE(t_buy_d.buy_num,0),";
            $data_sql .= "    t_order_d.num - COALESCE(t_buy_d.buy_num,0) AS ord_close_num,";
            $data_sql .= "    t_order_d.reason ";
            $data_sql .= " FROM ";
            $data_sql .= "    t_order_d ";

            $data_sql .= "    LEFT JOIN ";
            $data_sql .= "        (SELECT";
            $data_sql .= "            t_aorder_d.goods_id,";
            $data_sql .= "            t_aorder_h.arrival_day ";
            $data_sql .= "        FROM ";
            $data_sql .= "            t_aorder_h ";
            $data_sql .= "        INNER JOIN t_aorder_d ON t_aorder_h.aord_id = t_aorder_d.aord_id ";
            $data_sql .= "        WHERE ";
            $data_sql .= "            t_aorder_h.fc_ord_id = $ord_id";
            $data_sql .= "        )AS t_aorder ";
            $data_sql .= "    ON t_order_d.goods_id = t_aorder.goods_id ";
          
            $data_sql .= "    LEFT JOIN ";
            $data_sql .= "    (SELECT";        
            $data_sql .= "        ord_d_id,";
            $data_sql .= "        SUM(num) AS buy_num";
            $data_sql .= "    FROM ";    
            $data_sql .= "        t_buy_d";
            $data_sql .= "    GROUP BY ord_d_id";    
            $data_sql .= "    ) AS t_buy_d";    
            $data_sql .= "    ON t_order_d.ord_d_id = t_buy_d.ord_d_id ";

            $data_sql .= "    INNER JOIN t_order_h ON t_order_d.ord_id = t_order_h.ord_id ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤Ç¤Ï¤Ê¤¤
            if($ps_stat != '4'){
                $data_sql .= "    INNER JOIN t_goods ON t_order_d.goods_id = t_goods.goods_id ";
            }

            $data_sql .= "WHERE ";
            $data_sql .= "    t_order_d.ord_id = $ord_id ";
            $data_sql .= "AND ";
            $data_sql .= "    t_order_h.shop_id = $shop_id ";
            $data_sql .= "ORDER BY ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }else{
//              $data_sql .= "    t_goods.goods_cd;";
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }
        }else{
            /****************************/
            //È¯Ãí¥Ç¡¼¥¿+(Æþ²Ù¿ô¡¦È¯Ãí»Ä¡¦È¯Ãí´°Î»ÍýÍ³) Ãê½ÐSQL
            //­¢¶¯À©´°Î»¥Õ¥é¥°¡á£ô¡¡¤Î¾ì¹ç
            /****************************/
            $data_sql  = "SELECT ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                $data_sql .= "    t_order_d.goods_cd,";
            }else{
//            $data_sql .= "    t_goods.goods_cd,";
                $data_sql .= "    t_order_d.goods_cd,";
            }
            $data_sql .= "    t_order_d.goods_name,";
            $data_sql .= "    t_order_d.num,";            
            $data_sql .= "    t_order_d.buy_price,";
            $data_sql .= "    t_order_d.buy_amount,";
            $data_sql .= "    COALESCE(t_buy_d.buy_num,0),";
            $data_sql .= "    t_order_d.num - COALESCE(t_buy_d.buy_num,0) AS ord_close_num,";
            $data_sql .= "    t_order_d.reason ";
            $data_sql .= "FROM ";
            $data_sql .= "    t_order_d ";
          
            $data_sql .= "    LEFT JOIN ";
            $data_sql .= "    (SELECT";        
            $data_sql .= "        ord_d_id,";
            $data_sql .= "        SUM(num) AS buy_num";
            $data_sql .= "    FROM ";    
            $data_sql .= "        t_buy_d";
            $data_sql .= "    GROUP BY ord_d_id";    
            $data_sql .= "    ) AS t_buy_d";    
            $data_sql .= "    ON t_order_d.ord_d_id = t_buy_d.ord_d_id ";

            $data_sql .= "    INNER JOIN t_order_h ON t_order_d.ord_id = t_order_h.ord_id ";

            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤Ç¤Ï¤Ê¤¤
            if($ps_stat != '4'){
                $data_sql .= "    INNER JOIN t_goods ON t_order_d.goods_id = t_goods.goods_id ";
            }

            $data_sql .= "WHERE ";
            $data_sql .= "    t_order_d.ord_id = $ord_id ";
            $data_sql .= "AND ";
            $data_sql .= "    t_order_h.shop_id = $shop_id ";
            $data_sql .= "ORDER BY ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }else{
//            $data_sql .= "    t_goods.goods_cd;";
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }
        }
    }else{
        //È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¤«
        if($ord_stat == '2'){
            /****************************/
            //È¯Ãí¥Ç¡¼¥¿+(½Ð²ÙÍ½ÄêÆü) Ãê½ÐSQL
            //­£¶¯À©´°Î»¥Õ¥é¥°¡á£æ ¡õ È¯Ãí¾õ¶·¤¬È¯Ãí¼õÉÕ¡¡¤Î¾ì¹ç
            /****************************/
            $data_sql  = "SELECT ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                $data_sql .= "    t_order_d.goods_cd,";
            }else{
//            $data_sql .= "    t_goods.goods_cd,";
                $data_sql .= "    t_order_d.goods_cd,";
            }
            $data_sql .= "    t_order_d.goods_name,";
            $data_sql .= "    t_order_d.num,";            
            $data_sql .= "    t_order_d.buy_price,";
            $data_sql .= "    t_order_d.buy_amount, ";
            $data_sql .= "    t_aorder.arrival_day ";
            $data_sql .= "FROM ";
            $data_sql .= "    t_order_d ";

            $data_sql .= "    LEFT JOIN ";
            $data_sql .= "        (SELECT";
            $data_sql .= "            t_aorder_d.goods_id,";
            $data_sql .= "            t_aorder_h.arrival_day ";
            $data_sql .= "        FROM ";
            $data_sql .= "            t_aorder_h ";
            $data_sql .= "        INNER JOIN t_aorder_d ON t_aorder_h.aord_id = t_aorder_d.aord_id ";
            $data_sql .= "        WHERE ";
            $data_sql .= "            t_aorder_h.fc_ord_id = $ord_id";
            $data_sql .= "        )AS t_aorder ";
            $data_sql .= "    ON t_order_d.goods_id = t_aorder.goods_id ";

            $data_sql .= "    INNER JOIN t_order_h ON t_order_d.ord_id = t_order_h.ord_id ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤Ç¤Ï¤Ê¤¤
            if($ps_stat != '4'){
                $data_sql .= "    INNER JOIN t_goods ON t_order_d.goods_id = t_goods.goods_id ";
            }

            $data_sql .= "WHERE ";
            $data_sql .= "    t_order_d.ord_id = $ord_id ";
            $data_sql .= "AND ";
            $data_sql .= "    t_order_h.shop_id = $shop_id ";
            $data_sql .= "ORDER BY ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }else{
//            $data_sql .= "    t_goods.goods_cd;";
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }
        }else{
            /****************************/
            //È¯Ãí¥Ç¡¼¥¿Ãê½ÐSQL
            //­¤¶¯À©´°Î»¥Õ¥é¥°¡á£æ¡¡¤Î¾ì¹ç
            /****************************/
            $data_sql  = "SELECT ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                $data_sql .= "    t_order_d.goods_cd,";
            }else{
//            $data_sql .= "    t_goods.goods_cd,";
                $data_sql .= "    t_order_d.goods_cd,";
            }
            $data_sql .= "    t_order_d.goods_name,";
            $data_sql .= "    t_order_d.num,";            
            $data_sql .= "    t_order_d.buy_price,";
            $data_sql .= "    t_order_d.buy_amount ";
            $data_sql .= "FROM ";
            $data_sql .= "    t_order_d ";
            $data_sql .= "    INNER JOIN t_order_h ON t_order_d.ord_id = t_order_h.ord_id ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤Ç¤Ï¤Ê¤¤
            if($ps_stat != '4'){
                $data_sql .= "    INNER JOIN t_goods ON t_order_d.goods_id = t_goods.goods_id ";
            }

            $data_sql .= "WHERE ";
            $data_sql .= "    t_order_d.ord_id = $ord_id ";
            $data_sql .= "AND ";
            $data_sql .= "    t_order_h.shop_id = $shop_id ";
            $data_sql .= "ORDER BY ";
            //½èÍý¾õ¶·¤¬Æü¼¡¹¹¿·¸å¤«
            if($ps_stat == '4'){
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }else{
//            $data_sql .= "    t_goods.goods_cd;";
                //aoyama-n 2009-09-18
                #$data_sql .= "    t_order_d.goods_cd;";
                $data_sql .= "    t_order_d.line;";
            }
        }
    }
    $result = Db_Query($db_con,$data_sql);


    //aoyama-n 2009-09-18
    //ÃÍ°ú¥Õ¥é¥°Ãê½Ð
    $sql  = "SELECT ";
    $sql .= "    t_goods.discount_flg ";
    $sql .= "FROM ";
    $sql .= "    t_order_h INNER JOIN t_order_d ON t_order_h.ord_id = t_order_d.ord_id ";
    $sql .= "    INNER JOIN t_goods ON t_order_d.goods_id = t_goods.goods_id ";
    $sql .= "WHERE ";
    $sql .= "    t_order_d.ord_id = $ord_id ";
    $sql .= "AND ";
    $sql .= "    t_order_h.shop_id = $shop_id ";
    $sql .= "ORDER BY ";
    $sql .= "    t_order_d.line;";
    $sql_result = Db_Query($db_con,$sql);
    for($i = 0; $i < pg_numrows($sql_result); $i++){
        $discount_flg[] = pg_fetch_result ($sql_result, $i, "discount_flg");
    } 

    /****************************/
    //È¯Ãí¥Ç¡¼¥¿¡¼É½¼¨
    /****************************/
    //¹Ô¹àÌÜÉôÉÊ¤òºîÀ®
    $row_item = Get_Ord_Item($finish_flg,$ord_stat);
    //¹Ô¥Ç¡¼¥¿ÉôÉÊ¤òºîÀ®
    //aoyama-n 2009-09-18
    #$row_data = Get_Ord_Data($result,$finish_flg,$ord_stat);
    $row_data = Get_Ord_Data($result,$finish_flg,$ord_stat,$discount_flg);

}

/****************************/
//È¯Ãí¥Ø¥Ã¥À¡¼É½¼¨
/****************************/
//ÆüÉÕÀ¸À®
$form_ord_date    = explode('-',$h_data_list[0][1]);
$form_hope_day    = explode('-',$h_data_list[0][2]);
$form_arrival_day = explode('-',$h_data_list[0][3]);

$def_fdata["form_send_date"]                    =   $h_data_list[0][16];                  //È¯¿®Æü
$def_fdata["form_ord_no"]                       =   $h_data_list[0][0];                   //È¯ÃíNo.

$def_fdata["form_ord_time"]["y"]                =   $form_ord_date[0];                    //È¯ÃíÆü(Ç¯)
$def_fdata["form_ord_time"]["m"]                =   $form_ord_date[1];                    //È¯ÃíÆü(·î)
$def_fdata["form_ord_time"]["d"]                =   $form_ord_date[2];                    //È¯ÃíÆü(Æü)

if($form_hope_day[0] != NULL){
    $def_fdata["form_hope_day"]["y"]            =   $form_hope_day[0]." -";               //´õË¾Ç¼´ü(Ç¯)
    $def_fdata["form_hope_day"]["m"]            =   $form_hope_day[1]." -";               //´õË¾Ç¼´ü(·î)
    $def_fdata["form_hope_day"]["d"]            =   $form_hope_day[2];                    //´õË¾Ç¼´ü(Æü)
}

$def_fdata["form_arrival_day"]["y"]             =   $form_arrival_day[0];                 //Æþ²ÙÍ½ÄêÆü(Ç¯)
$def_fdata["form_arrival_day"]["m"]             =   $form_arrival_day[1];                 //Æþ²ÙÍ½ÄêÆü(·î)
$def_fdata["form_arrival_day"]["d"]             =   $form_arrival_day[2];                 //Æþ²ÙÍ½ÄêÆü(Æü)

//¥°¥ê¡¼¥ó»ØÄê¡õ±¿Á÷¶È¼ÔÉôÉÊºîÀ®
$text="";
//¥°¥ê¡¼¥ó»ØÄê¤µ¤ì¤Æ¤¤¤ë¤«
if($h_data_list[0][4] == 't'){
    $text[] =& $form->createElement("static","green_trans","","");
    $text[] =& $form->createElement("static","","","¡¡");
    $def_fdata["form_trans"]["green_trans"]     =  ($h_data_list[0][4] == 't')? ¥°¥ê¡¼¥ó»ØÄê¤¢¤ê : null;
}
$text[] =& $form->createElement("static","trans_name","","");
$def_fdata["form_trans"]["trans_name"]          =   $h_data_list[0][5];                      //±¿Á÷¶È¼Ô
$form->addGroup( $text,"form_trans","form_trans");
              
$def_fdata["form_client"]["cd1"]                =   $h_data_list[0][6];                      //»ÅÆþÀè    
//$def_fdata["form_client"]["cd2"]                =   $h_data_list[0][7];                          
$def_fdata["form_client"]["name"]               =   $h_data_list[0][7];                          

$def_fdata["form_direct_name"]                  =   $h_data_list[0][8];                      //Ä¾Á÷Àè
$def_fdata["form_ware_name"]                    =   $h_data_list[0][9];                     //ÁÒ¸Ë
$def_fdata["form_trade_ord"]                    =   $h_data_list[0][10];                     //¼è°ú¶èÊ¬
$def_fdata["form_c_staff_name"]                 =   $h_data_list[0][11];                     //Ã´Åö¼Ô
$def_fdata["form_note_your"]                    =   $h_data_list[0][13];                     //ÄÌ¿®Íó(»ÅÆþÀè)
$def_fdata["form_note_your2"]                   =   $h_data_list[0][17];                     //ÂèÆóÄÌ¿®Íó(»ÅÆþÀè)
$note_my                     =   $h_data_list[0][12];                     //ÄÌ¿®Íó(»ÅÆþÀè)
$def_fdata["form_buy_total"]                    =   number_format($h_data_list[0][14]);      //ÀÇÈ´¶â³Û
$def_fdata["form_buy_tax"]                      =   number_format($h_data_list[0][15]);      //¾ÃÈñÀÇ
$total_money                                    =   $h_data_list[0][14] + $h_data_list[0][15];  //ÀÇ¹þ¶â³Û
$def_fdata["form_buy_money"]                    =   number_format($total_money);  
$form->setDefaults($def_fdata);

/****************************/
//HTML¥Ø¥Ã¥À
/****************************/
$html_header = Html_Header($page_title);

/****************************/
//HTML¥Õ¥Ã¥¿
/****************************/
$html_footer = Html_Footer();

/****************************/
//¥á¥Ë¥å¡¼ºîÀ®
/****************************/
$page_menu = Create_Menu_h('buy','1');

/****************************/
//²èÌÌ¥Ø¥Ã¥À¡¼ºîÀ®
/****************************/
$page_header = Create_Header($page_title);

// Render´ØÏ¢¤ÎÀßÄê
$renderer =& new HTML_QuickForm_Renderer_ArraySmarty($smarty);
$form->accept($renderer);

//form´ØÏ¢¤ÎÊÑ¿ô¤òassign
$smarty->assign('form',$renderer->toArray());

//¤½¤ÎÂ¾¤ÎÊÑ¿ô¤òassign
$smarty->assign('var',array(
    'html_header'   => "$html_header",
    'page_menu'     => "$page_menu",
    'page_header'   => "$page_header",
    'html_footer'   => "$html_footer",
    'order_sheet'   => "$order_sheet",
    'load'          => "$load",
    'note_my'       => "$note_my",
    'freeze_flg'    => "$freeze_flg",
    'output_flg'    => "$output_flg",
));

$smarty->assign('item',$row_item);
$smarty->assign('row',$row_data);
//¥Æ¥ó¥×¥ì¡¼¥È¤ØÃÍ¤òÅÏ¤¹
$smarty->display(basename($_SERVER[PHP_SELF] .".tpl"));

?>
