 ...
 *      ・（2007-05-08）検索SQLの結合条件にonline_flgを追加＜morita-d＞
 *      ・（2007-05-31）検索条件に商品分類を追加＜morita-d＞
 * Rev.1.5（2010-05-12）初期表示に検索項目だけ表示する修正＜hashimoto-y＞
 *
 *
******************************/
...
// 検索フォーム初期値配列
$ary_form_list = array(
    "form_stat_check"   => array("stat1" => "1", "stat2" => "1", "stat3" => "1", "stat4" => "1", "stat5" => "1"),
    "form_rental_no"    => "",
...
// 検索条件復元
Restore_Filter2($form, "rental", "form_show_button", $ary_form_list, $ary_pass_list);
...
// ショップCD
Addelement_Client_64($form, "form_client", "ショップコード", "-");

// FCのショップ
$select_value = null;
$select_value = Select_Get($db_con, "rshop");
$form->addElement("select", "form_fc", "", $select_value, $g_form_option_select);
...
// 登録（ヘッダ）
$form->addElement("button", "input_btn", "登　録", "onClick=\"location.href='1-2-132.php'\"");
// 一覧（ヘッダ）
...
if ($post_flg == true && $err_flg != true){
    ...
    // ショップコード１
    $sql .= ($client_cd1 != null) ? "AND t_rental_h.shop_cd1 LIKE '$client_cd1%' \n" : null;
    // ショップコード２
    $sql .= ($client_cd2 != null) ? "AND t_rental_h.shop_cd2 LIKE '$client_cd2%' \n" : null;
    // ショップ名
    $sql .= ($fc != null) ? " AND t_rental_h.shop_id = $fc \n" : null;
    // 商品分類
...
$sql .= "       WHERE \n";
$sql .= "           t_rental_h.rental_id IS NOT NULL \n";
if ($stat_check1 == null && $stat_check2 == null && $stat_check3 == null && $stat_check4 == null && $stat_check5 == null){
...
$sql .= "   AND \n";
$sql .= "   t_count2.online_flg = t_rental_h.online_flg \n";
$sql .= "   AND \n";
$sql .= "   ( \n";
$sql .= "       ( \n";
...
$sql .= "   WHERE \n";
$sql .= "       t_rental_h.rental_id IS NOT NULL \n";
$sql .= "   AND (t_rental_h.regist_shop_id = $shop_id OR t_rental_h.online_flg = 't' )\n ";
...
for($x=0;$x<count($disp_data);$x++){
	...
	$disp_data2[$shop_num][4] = $disp_data2[$shop_num][4] + $disp_data[$x][18];    //ショップ提供額
}
	...
	while($shop_arr = each($n_data_list)){
		...
		while($client_arr = each($n_data_list[$shop])){
			...
			while($rental_arr = each($n_data_list[$shop][$client_arr[0]])){
				...
				//レンタル番号
				$html[$shop] .= "<td  align=\"left\" rowspan=".$rental[1]."><a href='".HEAD_DIR."sale/1-2-132.php?rental_id=".$rental[2]."'>".$rental[0]."</a></td>";
				//出荷日
				...
				for($x=0;$x<count($n_data_list[$shop][$client_arr[0]][$rental_arr[0]]);$x++){
					...
					//レンタル単価・金額
					$html[$shop] .= "<td  align=\"right\" >"
					.number_format($n_data_list[$shop][$client_arr[0]][$rental_arr[0]][$x][15],2)."<br>"
					.number_format($n_data_list[$shop][$client_arr[0]][$rental_arr[0]][$x][17],2)."</td>";
					
					//ユーザ単価・金額
					$html[$shop] .= "<td  align=\"right\" >"
					.number_format($n_data_list[$shop][$client_arr[0]][$rental_arr[0]][$x][16])."<br>"
					.number_format($n_data_list[$shop][$client_arr[0]][$rental_arr[0]][$x][18])."</td>";
					...
					$shop_money   = $shop_money + $n_data_list[$shop][$client_arr[0]][$rental_arr[0]][$x][18];  //ユーザ金額
			...
			$html[$shop] .= "    <td  >　</td>";
			$html[$shop] .= "    <td  >　</td>";
			$html[$shop] .= "    <td  align=\"right\">".number_format($rental_money)."<br>".number_format($shop_money)."</td>";
			$html[$shop] .= "    <td  >　</td>";
			$html[$shop] .= "</tr>";
			...
			$total_user = $total_user + $shop_money;
...
$total_shop   = number_format($total_shop);
$total_num    = number_format($total_num);
$total_rental = number_format($total_rental);
