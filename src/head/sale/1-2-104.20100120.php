...
for($i=0;$i<$num;$i++){
    ...
    //出荷回数
    $form->addElement('select', 'form_forward_times['.$i.']', "出荷回数", $select_page_arr,"onChange=\"javascript:Button_Submit('forward_num_flg','#','true')\"");
...
if($comp_flg != true){
    ...
    $button[] = $form->createElement(
            "submit","alert_ok","警告を無視して登録","onClick=\"javascript:return Dialogue('登録します。','#')\" $disabled"
            );
