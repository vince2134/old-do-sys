 ...
 * 履歴：
 * 　日付　　　　B票No.　　　　担当者　　　内容　
 * 　2006/12/13　      　　　　watanabe-k　請求更新取り消しがいつまでもできてしまうバグの修正
 ...
 *   2007/03/08　      　　　　watanabe-k  月次更新の日付以前で請求取り消しができないように変更
 *   2007/03/14                watanabe-k  再発行の処理を追加
 *   2007/03/28                watanabe-k　検索条件の復元処理を追加
 *   2007/04/16                watanabe-k　確定のチェックボックスを表示しない
 *   2007/07/05                watanabe-k　東陽以外の検索を追加
 */
...
if ($_POST["renew_flg"] == "1"){ 
    ...
    $sv_staff_cd        = $_POST["form_sv_staff"]["cd"];
    $sv_staff_select    = $_POST["form_sv_staff"]["select"];
    ...
    $data_state         = $_POST["form_data_state"];
...
/****************************/
/* 共通フォーム */
Search_Form_Claim_H($db_con, $form, $ary_form_list);
...
//抽出状況
$obj    =   null;
$obj[]  =&  $form->createElement("radio", null, null, "東陽以外","toyo");   
$obj[]  =&  $form->createElement("radio", null, null, "全て", "all");   
$form->addGroup($obj, "form_data_state", "", " ");
...
// 表示ボタン
$form->addElement("submit", "hyouji_button", "表　示", "onClick=\"return(Submit_If_Url('1-2-303.php', 'form_output_type'));\"");
...
$form->addElement("button", "hakkou_button", "請求書発行", "
    ...
    Submit_Blank_Window('1-2-307.php','請求書を発行します。');\" $disabled
");
...
if ($_POST["hyouji_button"] == "表　示"){
    ...
    /****************************/
    // ■担当者（SV）
    $err_msg = "担当者（SV） は数値のみ入力可能です。";
    Err_Chk_Num($form, "form_sv_staff", $err_msg);
...
// 処理
/****************************/
// 出力形式「画面」で表示ボタンが押された時の処理
if ($hyouji_button == "表　示" && $output_type == "1") {

    // なし

}else
// 出力形式「帳票」で表示ボタンが押された時の処理
...
/****************************/
// 共通検索テーブル
$html_s .= Search_Table_Claim_H($form);
// モジュール個別検索テーブル１
...
$html_s .= "    <col width=\"300px\">\n";
$html_s .= "    <tr>\n";
$html_s .= "        <td class=\"Td_Search_3\">抽出条件</td>\n";
$html_s .= "        <td class=\"Td_Search_3\">".$form->_elements[$form->_elementIndex["form_data_state"]]->toHtml()."</td>\n";
$html_s .= "    </tr>\n";
$html_s .= "</table>\n";
// ボタン
