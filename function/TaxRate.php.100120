...
class TaxRate
{
    ...
    {
        //·×¾åÆü¤¬Àµ¤·¤¯¤Ê¤¤¾ì¹ç¡¢½é´üÃÍ¤ÎËÜÆüÆüÉÕ¤òÀßÄê
        $y = date("Y");
        $m = date("m");
        $d = date("d");
    ...
    public function setOwnShopTaxRate()
    {
        ...
        $tax_rate_old       = pg_fetch_result($result, 0, 0);       //µì¾ÃÈñÀÇÎ¨
        $tax_rate_now       = pg_fetch_result($result, 0, 1);       //¸½¾ÃÈñÀÇÎ¨
        $tax_change_day_now = pg_fetch_result($result, 0, 2);       //¸½ÀÇÎ¨ÀÚÂØÆü
        $tax_rate_new       = pg_fetch_result($result, 0, 3);       //¿·¾ÃÈñÀÇÎ¨
        $tax_change_day_new = pg_fetch_result($result, 0, 4);       //¿·ÀÇÎ¨ÀÚÂØÆü

        //¸½ÀÇÎ¨ÀÚÂØÆü¤è¤êÁ°¤ÎÆüÉÕ¤Ïµì¾ÃÈñÀÇÎ¨
        //¿·¾ÃÈñÀÇÎ¨ÀÚÂØÆü¤¬¶õ¤Ç¤Ê¤¯¡¢¿·¾ÃÈñÀÇÎ¨°Ê¸å¤ÎÆüÉÕ¤Ï¿·¾ÃÈñÀÇÎ¨
        //¾åµ­°Ê³°¤Ï¸½¾ÃÈñÀÇÎ¨
    ...
    public function getClientTaxRate($client_id)
    {
        ...
        //²İÀÇ¶èÊ¬3¡ÊÈó²İÀÇ¡Ë¤Î¼è°úÀè¤Ï¾ÃÈñÀÇÎ¨0
        if ($tax_div == "3") {
            return 0;
        } else {
            return $this->shop_tax_rate;
